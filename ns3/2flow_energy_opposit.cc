...
/*
   ...
   to run the script use the folloiwng example

  
   ./waf --run "2flow_energy_opposit  --packetSize=1472 --dataRate=10Mbps --numPackets=849"
...
main (int argc, char *argv[])
{
  ...
  sinkAddress2 = InetSocketAddress (interfaces0.GetAddress (0), sinkPort2);
  ...
  ApplicationContainer sinkApps2 = packetSinkHelper2.Install (n0n1.Get (0));
  sinkApps2.Start (Seconds (0.0));
  sinkApps2.Stop (Seconds (1.0));

  Ptr<Socket> ns3UDPSocket1 = Socket::CreateSocket (n0n1.Get (0), UdpSocketFactory::GetTypeId ());
  Ptr<Socket> ns3UDPSocket2 = Socket::CreateSocket (n1n2.Get (1), UdpSocketFactory::GetTypeId ());
  ...
  app2->Setup (ns3UDPSocket2, sinkAddress2, packetSize, numPackets, DataRate (dataRate));
  n0n1.Get (0)->AddApplication (app2);
  app2->SetStartTime (Seconds (0.0));
  app2->SetStopTime (Seconds (1.0));
