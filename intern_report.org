...
**** Questions 
    - Taking the topology shown [[ifile:img/scenario.png][here]] (taken from ECOFEN) as an example, the objective of my internship is to use the ECOFEN energy model shown [[file:img/model.png][here]] in
      SimGrid simulator because SimGrid uses flow-based modeling
...
- For validating ECOFEN, the experiment found in section E (Per-Byte Total Energy) in this paper [[http://ieeexplore.ieee.org/document/5928833/][Profiling per-packet and per-byte energy consumption
...
- The shape of the plots shown above is not what I was expecting to see from a linear model, therefore I have decided to re-run another validation
  experiment this time with large data and using the other equations shown in [[file:figures/powervsdatarate.pdf][Figure 4]] in the paper - [[http://ieeexplore.ieee.org/document/5928833/][Profiling per-packet and per-byte energy
...
ggplot(all_data,aes(x=Throughput))+
   geom_point(aes(y = Cons_Node0, colour = "Simulated Node-(0 or 1)"))+
...
#+END_SRC
- Extract data from [[file:data/eco_val_modified_100.dat]]
#+CALL: extract_data(input_file="data/eco_val_modified_100.dat", output_file="data/eco_val_modified_100.csv",size=100) :exports code
- Extract data from file:data/eco_val_modified_576.dat
#+CALL: extract_data(input_file="data/eco_val_modified_576.dat", output_file="data/eco_val_modified_576.csv",size=576) : exports code
- Extract data from [[file:data/eco_val_modified_1000.dat]]
#+CALL: extract_data(input_file="data/eco_val_modified_1000.dat", output_file="data/eco_val_modified_1000.csv",size=1000) :exports code
- Extract data from [[file:data/eco_val_modified_1500.dat]]
#+CALL: extract_data(input_file="data/eco_val_modified_1500.dat", output_file="data/eco_val_modified_1500.csv",size=1500) :exports code
- Running the above calls produces [[file:data/eco_val_modified_100.csv]], [[file:data/eco_val_modified_576.csv]], [[file:data/eco_val_modified_1000.csv]], and
  [[file:data/eco_val_modified_1500.csv]].
- When we plot the above data we get the graph shown below
