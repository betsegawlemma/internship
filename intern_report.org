...
**** Experiment 16: Run time and memory usage as a function of link size (bw 10Mbps, lat 10ms, data size 100 MBytes, flow 2, link range {1, 2, 4, 6, 8})
...
for ((k=0; k<11; k=k+2));
 do
    ...
    do
       ...
       /usr/bin/time  -f "ElapsedTime %E ElapsedSec %e PeakMem %M AvgTotalMem %K" ./energy-link energy_consumption_${k}L_p.xml 2 ${bytesToSend} >> /home/bamersho/bamersho/data/perfLnk_sg_L${k}F2.raw 2>&1
       # run ecofen with /usr/bin/time
       /usr/bin/time  -f "ElapsedTime %E ElapsedSec %e PeakMem %M AvgTotalMem %K" ./energy-link energy_consumption_${k}L_p.xml 2 ${bytesToSend} --NS3 --cfg=network/model:NS3 >> /home/bamersho/bamersho/data/perfLink_ns3_L${k}F2.raw 2>&1
     done
done
#+END_SRC
...
PEAK_MEM_COL=5
AVG_TOTAL_MEM_COL=7
output_file_m = "data/perfLink1_"+version+"_F2.csv"  
for i in range(0, 8, 2):
    input_file_m = "data/perfLink1_"+version+"_L"+str(i)+"F2.raw" 
...
<<format_theme>>
   ex16_ns3 = read.csv("data/perfLink1_ns3_F2.csv", header=T)
   ex16_sg = read.csv("data/perfLink1_sg_F2.csv", header=T)
...
**** Experiment 21: Run time and memory usage as a function of data size (bw 50 Mbps, lat 10ms, data size [20,200] Mbytes, flow 2, link size 2)
...
<<format_theme>>
   ex21_ns3 = read.csv("data/perfDat_ns3_L2F2.csv", header=T)
   ex21_sg = read.csv("data/perfDat_sg_L2F2.csv", header=T)
