...
**** Questions
     - Conditions for proposing power consumption modeling
       ...
       - In a given data center, the power consumption of network equipment is about [[https://research.google.com/pubs/pub32980.html][2%]] to [[http://ieeexplore.ieee.org/document/5072138/][15%]], 
         therefore it can be safely ignored. However, I think at least the idle power consumption of switches can be incorporated in SimGrid using the <link> tags that represent switches
       - Having the above conditions, a linear models seems to fit into SimGrid's abstraction. Accordingly, I propose the following linear model for SimGrid, which is first proposed by [[https://research.google.com/pubs/pub32980.html][Fan et al's]] but the original
         model does not account for network energy consumption
       #+BEGIN_SRC LaTex
     ...
     - To estimate the power consumption of the whole network in a given large-scale network, we add to this sum the total idle power consumption of involved switches
     ...
     - Questions
       ...
       - Can implementing CPU utilization-based power consumption model in SimGrid be considered as scientific contribution? is not this typical software development task?
       - Do I really need to come up with a brand new model if this model gives reasonably accurate result? I noticed that the researchers propose a model after measuring power consumption of servers and 
         network equipment and after statistical analysis of the collected data.  
**** Work Planned
