...
- Running [[file:ns3/validate_ecofen_complete.cc][new NS-3 script]] as shown in the shell code block below produces files: [[file:data/eco_val_complete_576.dat]],
  [[file:data/eco_val_complete_1000.dat]], and [[file:data/eco_val_complete_1500.dat]].
...
[[file:figures/ecof_val_recieve.png]]
  - If we compare the simulated and the expected consumption using unpaired t-test for the middle plot, we get the following result. 
 #+BEGIN_SRC R :results output :var validation=validation :exports both 
       val_1000 = read.csv("data/eco_val_complete_1000.csv", header=T)
       attach(val_1000)
...
[[file:figures/ecof_val_send.png]]
  ...
  - If we compare the simulated and the expected consumption using unpaired t-test, we get the following result
 #+BEGIN_SRC R :results output :var validation=validation :exports both 
       val_1000 = read.csv("data/eco_val_complete_1000.csv", header=T)
       attach(val_1000)
...
- for both the sending and the receiving end, even though the statistical tests say the difference is significant, the 95% confidence intervals tells
  us that the absolute difference between the two consumption is not very far from zero. The 95% CI for the receiving end for the packet size 1000 is
  [0.2996378,0.3776349], and for the sending end is [0.1546320, 0.2746287]. 
