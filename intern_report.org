...
*** TODO A Power Benchmarking Framework for Network Devices #+NAME: benchmark
...
**** Questions
    ...
    #+begin_src latex
       ...
       P_equipment(t) = idle_equipment + EPI_equipment * load_equipment(t)
       
    #+end_src

       where, EPI_equipment is energy proportionality index. Which is computed by dividing the slope of the measured line to the slope of the ideal proportional line to as shown in [[benchmark]]
    - The EPI value for network equipment is available from the benchmark study referenced. For example edge LAN switch has ~25, core switch 15.4, and core router 19.8
    ...
    - The total energy consumption can be computed by integrating the power consumed by individual device during the transmission time
