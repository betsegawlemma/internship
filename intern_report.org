...
#+END_EXAMPLE
- The following script replaces the "$T" value in the above XML file with randomly generated value and runs two simulation scripts
#+BEGIN_SRC bash :results output :exports both 
#! /bin/bash
seed=234
RANDOM=$(seed)
...
with open(input_file) as fe:
    for line in fe:
       ...
       if line.startswith("Time"):
           list = line.split()
           if done is not True and int(list[NODE_COL])>0:
...
: (Intercept)     Traffic 
:   0.1513248   0.1525936 
- From this we get our simple linear equation for the data transfer time as a function of data transferred 
#+BEGIN_EXAMPLE
...
#+END_EXAMPLE
  - we want to replace the $L place-holder with randomly generated latency value using the following script
#+BEGIN_SRC bash :results output :exports both 
#! /bin/bash
seed=33
RANDOM=$(seed)
...
: (Intercept)     Latency 
:    0.000135    0.168000 
- From this we get our simple linear equation for the data transfer time as a function of latency
#+BEGIN_EXAMPLE
  Transfer_Time = 0.168000 * Latency + 0.000135
#+END_EXAMPLE
- The two equations will combined give us the time required to transferred a given data at a given latency for a fixed bandwidth and packet size
