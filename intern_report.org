...
#+BEGIN_SRC R :results output graphics :file figures/expectedvssimulatedlinear.pdf :exports both :noweb yes 
<<format_theme>>
...
#+RESULTS:
[[file:figures/expectedvssimulatedlinear.pdf]]
...
#+BEGIN_SRC R :results output graphics :file figures/expectedvssimulatedreceiving.pdf :exports both :noweb yes
<<format_theme>>
...
#+RESULTS:
[[file:figures/expectedvssimulatedreceiving.pdf]]
...
ggplot(ex1_ns3,aes(x=Traffic))+
   geom_point(aes(y = Transfer_Time, ),size=1.3)+
...
** Week 2 June
**** Experiment 7: 2 Nodes, 1 Link, 1-flow, bw=100Mbps, lat=10ms
***** Directly on ECOFEN in NS-3, we used NS-3's bulk send tcp application with maximum bytes to send between 1 and 200 MBytes
#+BEGIN_SRC bash :results output :exports both
#! /bin/bash
cd ~/ns3ws/source/ns-3.26/ 
RANDOM=234
for ((k=0; k<40; k++)); do
  traffic=$(((RANDOM % 200)+1)) # traffic in B between 1 and 200
  maxBytes=$((traffic * (10**6))) # traffic in MB between 1 and 200
  # run ECOFEN
  ./waf --run "scratch/2Nodes1Link --maxBytes=${maxBytes}" >> ~/internship/data/2Nodes1Link_l.raw 2>&1
done
#+END_SRC
***** Extract Ecofen's data
#+NAME: extract_ns3_data_3
#+BEGIN_SRC python :results silent dir:~/internship :var input_file="" :var output_file="" 
import sys
...
SEND_BYTES_COL=1
RECEIVED_BYTES_COL=3
TRANSFER_TIME_COL = 5

# data variable holds one line of data formated as follows 
# TrafficSend,TotalBytesReceived,AveragePower,TotalTimeTaken,TotalEnergy
data = []
done = False
...
with open(input_file) as fe:
    for line in fe:
       if line.startswith("Time") and done is not True:
           ...
           prv_time = cur_time
           total_power+=float(list[POWER_COL])
       if line.startswith("TotalBytesSend") and done is not True:
           list = line.split()
           if number_of_power_values > 0:
               average_power = total_power/number_of_power_values
           send_bytes = (float(list[SEND_BYTES_COL])/1E6) 
           data.append(str(send_bytes))
           data.append(list[RECEIVED_BYTES_COL])
           data.append(str(average_power))
           transfer_time = float(list[TRANSFER_TIME_COL])
           total_energy = average_power * transfer_time
           data.append(str(transfer_time))
       ...
       if len(data) == 5 and done is True:
           with open(output_file,"a") as of:
               if header: # write the header part
                   of.write("%s\n" % "TrafficSend,TotalBytesReceived,AveragePower,TotalTimeTaken,TotalEnergy")
                   header = False
               ...
               cur_time = 0
#+END_SRC
#+CALL: extract_ns3_data_3(input_file="data/2Nodes1Link_l.raw", output_file="data/2Nodes1Link_l.csv")
***** SimGrid, 1 link in SimGrid is assumed to correspond to two NIC devices in NS-3
 #+BEGIN_SRC bash :results output :exports both 
#! /bin/bash
RANDOM=234
cd ~/simgrid/link_energy
for ((k=0; k<40; k++)); do
  traffic=$(((RANDOM % 200)+1)) # traffic in MBytes between 1 and 200
  # edit simgrid's deployment file to add traffic value
  cat energy_consumption_1F_d.xml |sed -e "s/\$T/${traffic}E6/" > t_energy_consumption_d.xml
  # run simgrid
  ./sg_energy_consumption energy_consumption_2H1L_p.xml t_energy_consumption_d.xml >> ~/internship/data/ex7_sg_2H1L1F.raw 2>&1
done
#+END_SRC     

#+RESULTS:

***** Extract SimGrid data
#+NAME: extract_sg_data_2
#+BEGIN_SRC python :results silent dir:~/internship :var input_file="" :var output_file="" 
import sys
...
# data variable holds one line of data formated as follows 
# TrafficSend, SgAveragePower, SgTransferTime, SgTotalEnergy
data = []
    ...
    for line in fe:
       ...
       if len(data) == 4 and done is True:
           with open(output_file,"a") as of:
               ...
               traffic = 0.0
#+END_SRC
#+CALL: extract_sg_data_2(input_file="data/ex7_sg_2H1L1F.raw", output_file="data/ex7_sg_2H1L1F.csv")
***** Result comparison
#+BEGIN_SRC R :results output graphics :file figures/ex7_energy_sg_vs_ns3.png :exports both :noweb yes
<<format_theme>>
   ex7_ns3 = read.csv("data/2Nodes1Link_l.csv", header=T)
   ex7_sg = read.csv("data/ex7_sg_2H1L1F.csv", header=T)
   sg_ns3 = merge(ex7_ns3,ex7_sg, by=c("TrafficSend"))
   attach(sg_ns3)
   ggplot(sg_ns3,aes(x=TrafficSend))+
   geom_point(aes(y = TotalEnergy, colour = "ECOFEN"),size=1.3)+
   geom_point(aes(y = SgTotalEnergy, colour = "SimGrid"),size=1.3)+
   ylab(label="Energy Consumption in Joule")+
   xlab(label="Transferred data in MBytes")+
   scale_colour_Publication()+
   theme_Publication()
...
#+RESULTS:
[[file:figures/ex7_energy_sg_vs_ns3.png]]

**** Experiment 8: 4 Nodes, 3 Link, 1-flow, bw=100Mbps, lat=10ms
***** Directly on ECOFEN, we used NS-3's bulk send tcp application with maximum bytes to send between 1 and 200 MBytes
#+BEGIN_SRC bash :results output :exports both
#! /bin/bash
cd ~/ns3ws/source/ns-3.26/
RANDOM=234
for ((k=0; k<40; k++)); do
  traffic=$(((RANDOM % 200)+1)) # traffic in MB between 1 and 200
  maxBytes=$((traffic * (10**6))) # traffic in MB between 1 and 200 
  # run ECOFEN
  ./waf --run "scratch/4Nodes3Links --maxBytes=${maxBytes}" >> ~/internship/data/4Nodes3Links_l.raw 2>&1
done
#+END_SRC
***** Extract Ecofen's data
#+CALL: extract_ns3_data_3(input_file="data/4Nodes3Links_l.raw", output_file="data/4Nodes3Links_l.csv")
***** SimGrid, 3 links in SimGrid assumed to correspond to 6 NIC's in NS-3, hence 4 Nodes with three links in between
#+BEGIN_SRC bash :results output :exports both 
#! /bin/bash
RANDOM=234
cd ~/simgrid/link_energy
for ((k=0; k<40; k++)); do
  ...
  cat energy_consumption_1F_d.xml |sed -e "s/\$T/${traffic}E6/" > t_energy_consumption_d.xml
  # run simgrid
  ./sg_energy_consumption energy_consumption_2H3L_p.xml t_energy_consumption_d.xml >> ~/internship/data/ex8_sg_2H3L1F.raw 2>&1
done
#+END_SRC

#+RESULTS:
#+CALL: extract_sg_data_2(input_file="data/ex8_sg_2H3L1F.raw", output_file="data/ex8_sg_2H3L1F.csv")
***** Result comparison
#+BEGIN_SRC R :results output graphics :file figures/ex8_energy_sg_vs_ns3.png :exports both :noweb yes
<<format_theme>>
   ex8_ns3 = read.csv("data/4Nodes3Links_l.csv", header=T)
   ex8_sg = read.csv("data/ex8_sg_2H3L1F.csv", header=T)
   sg_ns3 = merge(ex8_ns3,ex8_sg, by=c("TrafficSend"))
   attach(sg_ns3)
   ggplot(sg_ns3,aes(x=TrafficSend))+
   geom_point(aes(y = TotalEnergy, colour = "ECOFEN"),size=1.3)+
   geom_point(aes(y = SgTotalEnergy, colour = "SimGrid"),size=1.3)+
...
#+RESULTS:
[[file:figures/ex8_energy_sg_vs_ns3.png]]
**** Exercise 9: SimGrid(2 nodes, 1 links, 2 flow), ECOFEN(2 nodes, 1 links, 2 flow)
***** ECOFEN RUN Script
#+BEGIN_SRC bash :results output :exports both 
#! /bin/bash
...
RANDOM=234
for ((k=0; k<40; k++)); do
  traffic1=$(((RANDOM % 100)+1)) # traffic in B between 1 and 100
  traffic2=$(((RANDOM % 100)+1)) # traffic in B between 1 and 100
  maxBytes1=$((traffic1 * (10**6))) # traffic in MB between 1 and 100
  maxBytes2=$((traffic2 * (10**6))) # traffic in MB between 1 and 100
  # run ECOFEN
  ./waf --run "scratch/2Nodes1Link2F --maxBytes1=${maxBytes1} --maxBytes2=${maxBytes2}" >> ~/data/2Nodes1Link2F.raw 2>&1
done
#+END_SRC
***** Extract ECOFEN's data
#+NAME: extract_ns3_data_4
#+BEGIN_SRC python :results silent dir:~/internship :var input_file="" :var output_file="" 
import sys
...
with open(input_file) as fe:
    for line in fe:
       if line.startswith("Time") and done is not True:
           ...
           prv_time = cur_time
           total_power+=float(list[POWER_COL])
       if line.startswith("TotalBytesSend1") and done is not True:
           list = line.split()
           if number_of_power_values > 0:
               average_power = total_power/number_of_power_values
           send_bytes = (float(list[SEND_BYTES_COL])/1E6) 
       ...
       if len(data) == 5 and done is True:
           with open(output_file,"a") as of:
               ...
               cur_time = 0
#+END_SRC
#+CALL: extract_ns3_data_4(input_file="data/2Nodes1Link2F.raw", output_file="data/2Nodes1Link2F.csv")
***** SimGrid's run script
#+BEGIN_SRC bash :results output :exports both
#! /bin/bash
RANDOM=234
cd ~/simgrid/link_energy
for ((k=0; k<40; k++)); do
  traffic1=$(((RANDOM % 100)+1)) # traffic in MBytes between 1 and 100
  traffic2=$(((RANDOM % 100)+1)) # traffic in MBytes between 1 and 100
  # edit simgrid's deployment file to add traffic value
  cat energy_consumption_2F_d.xml |sed -e "s/\$T1/${traffic1}E6/" |sed -e "s/\$T2/${traffic2}E6/" > t_energy_consumption_d.xml
  # run simgrid
  ./sg_energy_consumption energy_consumption_2H1L_p.xml t_energy_consumption_d.xml >> ~/internship/data/ex9_sg_2H1L2F.raw 2>&1
done
#+END_SRC
***** Extract SimGrid's data
#+CALL: extract_sg_data_2(input_file="data/ex9_sg_2H1L2F.raw", output_file="data/ex9_sg_2H1L2F.csv")
***** Compare results
#+BEGIN_SRC R :results output graphics :file figures/ex9_energy_sg_vs_ns3.png :exports both :noweb yes
<<format_theme>>
   ex9_ns3 = read.csv("data/2Nodes1Link2F.csv", header=T)
   ex9_sg = read.csv("data/ex9_sg_2H1L2F.csv", header=T)
   sg_ns3 = merge(ex9_ns3,ex9_sg, by=c("TrafficSend"))
   attach(sg_ns3)
   ggplot(sg_ns3,aes(x=TrafficSend))+
   geom_point(aes(y = TotalEnergy, colour = "ECOFEN"),size=1.3)+
   geom_point(aes(y = SgTotalEnergy, colour = "SimGrid"),size=1.3)+
...
#+RESULTS:
[[file:figures/ex9_energy_sg_vs_ns3.png]]

**** Experiment 10: 1 link, 4 flows, with TCP-version CM02, bw 10Mbps, lat 10ms
#+BEGIN_SRC bash :results silent :exports both 
#! /bin/bash
RANDOM=234
cd ~/simgrid/examples/msg/energy-link
for ((k=0; k<40; k++)); do
  randVal=$(((RANDOM % 100)+20)) # random value between 20 and 100
  bytesToSend=$((randVal * (10**6))) # bytes to send in MBytes between 20 and 100
  # run simgrid
   ./energy-link energy_consumption_1L_p.xml 4 ${bytesToSend} >> ~/internship/data/ex10_sg_1L4F.raw 2>&1
  # run ecofen
   #./energy-link energy_consumption_1L_p.xml 4 ${bytesToSend} --NS3 --cfg=network/model:NS3 >> ~/internship/data/ex10_ns3_1L4F.raw 2>&1
done
#+END_SRC
***** Extract simgrid's data
#+CALL: extract_sg_data_3(input_file="data/ex10_sg_1L4F.raw", output_file="data/ex10_sg_1L4F.csv",flow=4) :exports both
***** Extract ecofen's data
#+CALL: extract_ns3_data_5(input_file="data/ex10_ns3_1L4F.raw", output_file="data/ex10_ns3_1L4F.csv",flow=4) :exports both
***** Compare results
#+BEGIN_SRC R :results output graphics :file figures/ex10_energy_sg_vs_ns3.png :exports both :noweb yes

<<format_theme>>

   ex10_ns3 = read.csv("data/ex10_ns3_1L4F.csv", header=T)
   ex10_sg = read.csv("data/ex10_sg_1L4F.csv", header=T)
   sg_ns3 = cbind(ex10_ns3,ex10_sg, by=c("BytesSend"))
   attach(sg_ns3)
   ggplot(sg_ns3,aes(x=BytesSend))+
   geom_point(aes(y = TotalEnergy, colour = "ECOFEN"),size=1.3)+
   geom_point(aes(y = SgTotalEnergy, colour = "SimGrid"),size=1.3)+
   ylab(label="Energy Consumption in Joule")+
   xlab(label="Data size in MBytes")+
   scale_colour_Publication()+
   theme_Publication()
#+END_SRC

#+RESULTS:
[[file:figures/ex10_energy_sg_vs_ns3.png]]
***** Statistical comparison
#+BEGIN_SRC R :results output :var validation=validation :exports both 
   ex10_ns3 = read.csv("data/ex10_ns3_1L4F.csv", header=T)
   ex10_sg = read.csv("data/ex10_sg_1L4F.csv", header=T)
   sg_ns3 = merge(ex10_ns3,ex10_sg, by=c("BytesSend"))
   attach(sg_ns3)
   t.test(TotalEnergy,SgTotalEnergy,paired=FALSE,conf.level=0.95)
   mean(log10(TotalEnergy)-log10(SgTotalEnergy))
  (exp(mean(log10(SgTotalEnergy)-log10(TotalEnergy)))) - 1
#+END_SRC
...
data:  TotalEnergy and SgTotalEnergy
t = -0.10452, df = 121.99, p-value = 0.9169
alternative hypothesis: true difference in means is not equal to 0
95 percent confidence interval:
 -720.1114  647.8859
sample estimates:
...
**** Experiment 11: Flows 1, Links 1, with TCP-version CM02, bw 10Mbps, lat 10ms
#+BEGIN_SRC bash :results silent :exports both 
#! /bin/bash
RANDOM=234
cd ~/simgrid/examples/msg/energy-link
for ((k=0; k<40; k++)); do
  randVal=$(((RANDOM % 500)+20)) # random value between 20 and 500
  bytesToSend=$((randVal * (10**6))) # bytes to send in MBytes between 20 and 500
  # run simgrid
   ./energy-link energy_consumption_1L_p.xml 1 ${bytesToSend} >> ~/internship/data/ex11_sg_1L1F.raw 2>&1
  # run ecofen
   ./energy-link energy_consumption_1L_p.xml 1 ${bytesToSend} --NS3 --cfg=network/model:NS3 >> ~/internship/data/ex11_ns3_1L1F.raw 2>&1
done
#+END_SRC
***** Extract SimGrids' data
#+NAME: extract_sg_data_3
#+BEGIN_SRC python :results silent dir:~/internship :var input_file="" :var output_file="" :var flow=1
import sys
...
# data variable holds one line of data formated as follows 
# BytesSend, SgAveragePower, SgTransferTime, SgTotalEnergy
data = []
...
with open(input_file) as fe:
    for line in fe:
       if " Send " in line: 
           ...
           list = line.split()
           traffic += (float(list[TRAFFIC_COL])/1E6) * flow
       if "SgTotalEnergy" in line:
           done = True
           list = line.split()
           data.append(str(traffic))
       ...
       if len(data) == 4 and done is True:
           with open(output_file,"a") as of:
               ...
               traffic = 0.0
#+END_SRC
#+CALL: extract_sg_data_3(input_file="data/ex11_sg_1L1F.raw", output_file="data/ex11_sg_1L1F.csv",flow=1)
***** Extract Ecofen's data
#+NAME: extract_ns3_data_5
#+BEGIN_SRC python :results silent dir:~/internship :var input_file="" :var output_file="" :var flow=1 
import sys
...
# data variable holds one line of data formated as follows 
# BytesSend,AveragePower,TotalTimeTaken,TotalEnergy
data = []
done = False traffic = 0.0
...
with open(input_file) as fe:
    for line in fe:
       ...
       if line.startswith("Time") and done is not True:
           list = line.split()
           cur_time = float(list[TIME_COL])
           if prv_time != cur_time: 
               number_of_power_values+=1
           prv_time = cur_time
           if cur_time > 0.0:
               total_power+=float(list[POWER_COL])
       if "receiver done" in line and done is not True:
           list = line.split()
           if number_of_power_values > 0:
               average_power = total_power/number_of_power_values
           data.append(str(traffic))
       ...
       if len(data) == 4 and done is True:
           with open(output_file,"a") as of:
               ...
               cur_time = 0
#+END_SRC
#+CALL: extract_ns3_data_5(input_file="data/ex11_ns3_1L1F.raw", output_file="data/ex11_ns3_1L1F.csv",flow=1)
***** Compare the result
#+BEGIN_SRC R :results output graphics :file figures/ex11_energy_sg_vs_ns3.png :exports both :noweb yes

<<format_theme>>

   ex11_ns3 = read.csv("data/ex11_ns3_1L1F.csv", header=T)
   ex11_sg = read.csv("data/ex11_sg_1L1F.csv", header=T)
   sg_ns3 = merge(ex11_ns3,ex11_sg, by=c("BytesSend"))
   attach(sg_ns3)
   ggplot(sg_ns3,aes(x=BytesSend))+
   geom_point(aes(y = TotalEnergy, colour = "ECOFEN"),size=1.3)+
   geom_point(aes(y = SgTotalEnergy, colour = "SimGrid"),size=1.3)+
...
#+RESULTS:
[[file:figures/ex11_energy_sg_vs_ns3.png]]
***** Statistical comparison 
#+BEGIN_SRC R :results output :var validation=validation :exports both 
   ex11_ns3 = read.csv("data/ex11_ns3_1L1F.csv", header=T)
   ex11_sg = read.csv("data/ex11_sg_1L1F.csv", header=T)
...
data:  TotalEnergy and SgTotalEnergy
t = -0.057422, df = 77.996, p-value = 0.9544
alternative hypothesis: true difference in means is not equal to 0
95 percent confidence interval:
 -1156.292  1091.460
sample estimates:
...
**** Exercise 12: Flow 2, Link 1, bw 10Mbps, lt 10ms, TCP version CM02
#+BEGIN_SRC bash :results silent :exports both 
#! /bin/bash
RANDOM=234
cd ~/simgrid/examples/msg/energy-link
for ((k=0; k<40; k++)); do
  randVal=$(((RANDOM % 500)+20)) # random value between 20 and 500
  bytesToSend=$((randVal * (10**6))) # traffic in MB between 20 and 500
  # run simgrid
   ./energy-link energy_consumption_1L_p.xml 2 ${bytesToSend} >> ~/internship/data/ex12_sg_1L2F.raw 2>&1
  # run ecofen
   ./energy-link energy_consumption_1L_p.xml 2 ${bytesToSend} --NS3 --cfg=network/model:NS3 >> ~/internship/data/ex12_ns3_1L2FL.raw 2>&1
done
#+END_SRC   
***** Extract simgrid's data
#+CALL: extract_sg_data_3(input_file="data/ex12_sg_1L2F.raw", output_file="data/ex12_sg_1L2F.csv",flow=2) :exports both
***** Extract ecofen's data
#+CALL: extract_ns3_data_5(input_file="data/ex12_ns3_1L2FL.raw", output_file="data/ex12_ns3_1L2FL.csv",flow=2) :exports both
***** Compare results
#+BEGIN_SRC R :results output graphics :file figures/ex12_energy_sg_vs_ns3.png :exports both :noweb yes

<<format_theme>>

   ex12_ns3 = read.csv("data/ex12_ns3_1L2FL.csv", header=T)
   ex12_sg = read.csv("data/ex12_sg_1L2F.csv", header=T)
   sg_ns3 = merge(ex12_ns3,ex12_sg, by=c("BytesSend"))
   attach(sg_ns3)
   ggplot(sg_ns3,aes(x=BytesSend))+
   geom_point(aes(y = TotalEnergy, colour = "ECOFEN"),size=1.3)+
   geom_point(aes(y = SgTotalEnergy, colour = "SimGrid"),size=1.3)+
...
#+RESULTS:
[[file:figures/ex12_energy_sg_vs_ns3.png]]
***** Statistical comparison
#+BEGIN_SRC R :results output  :exports both 
   ex12_ns3 = read.csv("data/ex12_ns3_1L2FL.csv", header=T)
   ex12_sg = read.csv("data/ex12_sg_1L2F.csv", header=T)
...
data:  TotalEnergy and SgTotalEnergy
t = -0.058866, df = 77.996, p-value = 0.9532
alternative hypothesis: true difference in means is not equal to 0
95 percent confidence interval:
 -2314.214  2181.289
sample estimates:
...
**** Experiment 13: Flow 1, Links 3, bw 10 Mbps, lat 10ms
#+BEGIN_SRC bash :results silent :exports both 
#! /bin/bash
RANDOM=234
cd ~/simgrid/examples/msg/energy-link
for ((k=0; k<40; k++)); do
  randVal=$(((RANDOM % 200)+20)) # random value between 20 and 200
  bytesToSend=$((randVal * (10**6))) # traffic in MB between 20 and 200
  # run simgrid
   ./energy-link energy_consumption_3L_p.xml 1 ${bytesToSend} >> ~/internship/data/ex13_sg_3L1F.raw 2>&1
  # run ecofen
   ./energy-link energy_consumption_3L_p.xml 1 ${bytesToSend} --NS3 --cfg=network/model:NS3 >> ~/internship/data/ex13_ns3_3L1FL.raw 2>&1
done
#+END_SRC 
***** Extract simgrid's data  
#+CALL: extract_sg_data_3(input_file="data/ex13_sg_3L1F.raw", output_file="data/ex13_sg_3L1F.csv",flow=1) 
***** Extract ecofen's data 
#+CALL: extract_ns3_data_5(input_file="data/ex13_ns3_3L1FL.raw", output_file="data/ex13_ns3_3L1FL.csv",flow=1)
***** Compare results
#+BEGIN_SRC R :results output graphics :file figures/ex13_energy_sg_vs_ns3.png :exports both :noweb yes

<<format_theme>>

   ex13_ns3 = read.csv("data/ex13_ns3_3L1FL.csv", header=T)
   ex13_sg = read.csv("data/ex13_sg_3L1F.csv", header=T)
   sg_ns3 = merge(ex13_ns3,ex13_sg, by=c("BytesSend"))
   attach(sg_ns3)
   ggplot(sg_ns3,aes(x=BytesSend))+
   geom_point(aes(y = TotalEnergy, colour = "ECOFEN"),size=1.3)+
   geom_point(aes(y = SgTotalEnergy, colour = "SimGrid"),size=1.3)+
...
#+RESULTS:
[[file:figures/ex13_energy_sg_vs_ns3.png]]
***** Statistical comparison
#+BEGIN_SRC R :results output :var validation=validation :exports both 
   ex13_ns3 = read.csv("data/ex13_ns3_3L1FL.csv", header=T)
   ex13_sg = read.csv("data/ex13_sg_3L1F.csv", header=T)
...
data:  TotalEnergy and SgTotalEnergy
t = -0.047469, df = 85.996, p-value = 0.9622
alternative hypothesis: true difference in means is not equal to 0
95 percent confidence interval:
 -1024.9558   977.1484
sample estimates:
...
**** Experiment 14: Flow 2, Links 3, bw 100Mbps, lat 10ms, flows 2 (Testing if the CM02 version works on higher bandwidth)
#+BEGIN_SRC bash :results silent :exports both 
#! /bin/bash
RANDOM=234
cd ~/simgrid/examples/msg/energy-link
for ((k=0; k<40; k++)); do
  randVal=$(((RANDOM % 100)+20)) # random value between 20 and 100
  bytesToSend=$((randVal * (10**6))) # traffic in MB between 20 and 100
  # run simgrid
   ./energy-link energy_consumption_3L_p.xml 2 ${bytesToSend} >> ~/internship/data/ex14_sg_3L2F.raw 2>&1
  # run ecofen
   ./energy-link energy_consumption_3L_p.xml 2 ${bytesToSend} --NS3 --cfg=network/model:NS3 >> ~/internship/data/ex14_ns3_3L2FL.raw 2>&1
done
#+END_SRC 
***** Extract simgrid's data  
#+CALL: extract_sg_data_3(input_file="data/ex14_sg_3L2F.raw", output_file="data/ex14_sg_3L2F.csv",flow=2) 
***** Extract ecofen's data 
#+CALL: extract_ns3_data_5(input_file="data/ex14_ns3_3L2FL.raw", output_file="data/ex14_ns3_3L2FL.csv",flow=2)
***** Compare results
#+BEGIN_SRC R :results output graphics :file figures/ex14_energy_sg_vs_ns3.png :exports both :noweb yes
<<format_theme>>
   ex14_ns3 = read.csv("data/ex14_ns3_3L2FL.csv", header=T)
   ex14_sg = read.csv("data/ex14_sg_3L2F.csv", header=T)
   sg_ns3 = merge(ex14_ns3,ex14_sg, by=c("BytesSend"))
   attach(sg_ns3)
   ggplot(sg_ns3,aes(x=BytesSend))+
   geom_point(aes(y = TotalEnergy, colour = "ECOFEN"),size=1.3)+
   geom_point(aes(y = SgTotalEnergy, colour = "SimGrid"),size=1.3)+
...
#+RESULTS:
[[file:figures/ex14_energy_sg_vs_ns3.png]]
***** Statistical comparison
#+BEGIN_SRC R :results output :var validation=validation :exports both 
   ex14_ns3 = read.csv("data/ex14_ns3_3L2FL.csv", header=T)
   ex14_sg = read.csv("data/ex14_sg_3L2F.csv", header=T)
...
data:  TotalEnergy and SgTotalEnergy
t = 13.094, df = 76.956, p-value < 2.2e-16
alternative hypothesis: true difference in means is not equal to 0
95 percent confidence interval:
 1191.27 1618.59
sample estimates:
...
**** Experiment resolution: Confirming ecofen's data point resolution effect 
#+NAME: extract_ns3_data_time_resolution
#+BEGIN_SRC python :results silent dir:~/internship :var input_file="" :var output_file="" 
import sys
...
with open(input_file) as ft:
    for line in ft:
        if line.startswith("TotalBytesSend1"):
            list = line.split()
            time_val.append(list[TRANSFER_TIME_COL])
...
# data variable holds one line of data formated as follows 
# BytesSend,TotalBytesReceived,AveragePower,TotalTimeTaken,TotalEnergy
data = []
done = False
    ...
    for line in fe:
       if line.startswith("Time") and done is not True:
           ...
           if prv_time != cur_time: 
               number_of_power_values+=1
           prv_time = cur_time
           if cur_time > 0.0 and cur_time <= time_val[tc]:
               total_power+=float(list[POWER_COL])
       if line.startswith("TotalBytesSend1") and done is not True:
           list = line.split()
           if number_of_power_values > 0:
               average_power = total_power/number_of_power_values
           send_bytes = (float(list[SEND_BYTES_COL])/1E6)*2 
           receive_bytes = (float(list[RECEIVED_BYTES_COL])/1E6)*2
           data.append(str(send_bytes))
           data.append(str(receive_bytes))
           data.append(str(average_power))
           transfer_time = float(list[TRANSFER_TIME_COL])
           total_energy = average_power * transfer_time
           data.append(str(transfer_time))
...
#+END_SRC
***** Extract ecofen data
#+CALL: extract_ns3_data_time_resolution(input_file="data/2Nodes1Link2F2S.raw", output_file="data/2Nodes1Link2F2S.csv") 
***** Run simgrid
#+BEGIN_SRC bash :results silent :exports both 
#! /bin/bash
RANDOM=234
cd ~/simgrid/examples/msg/energy-link
for ((k=0; k<40; k++)); do
  randVal=$(((RANDOM % 100)+1)) # random value between 20 and 500
  bytesToSend=$((randVal * (10**6))) # bytes to send in MBytes between 20 and 500
  # run simgrid
   ./energy-link energy_consumption_1L_p.xml 2 ${bytesToSend} >> ~/internship/data/exR_sg_1L2F.raw 2>&1
done
#+END_SRC
***** Extract simgrid's data
#+CALL: extract_sg_data_3(input_file="data/exR_sg_1L2F.raw", output_file="data/exR_sg_1L2F.csv",flow=2)
***** Compare results
#+BEGIN_SRC R :results output graphics :file figures/exR_energy_sg_vs_ns3.png :exports both :noweb yes
<<format_theme>>
   exR_ns3 = read.csv("data/2Nodes1Link2F2S.csv", header=T)
   exR_sg = read.csv("data/exR_sg_1L2F.csv", header=T)
   sg_ns3 = merge(exR_ns3,exR_sg, by=c("BytesSend"))
   attach(sg_ns3)
   ggplot(sg_ns3,aes(x=BytesSend))+
   geom_point(aes(y = TotalEnergy, colour = "ECOFEN"),size=1.3)+
   geom_point(aes(y = SgTotalEnergy, colour = "SimGrid"),size=1.3)+
...
#+RESULTS:
[[file:figures/exR_energy_sg_vs_ns3.png]]
***** Statistical comparison
#+BEGIN_SRC R :results output :var validation=validation :exports both 
   exR_ns3 = read.csv("data/2Nodes1Link2F2S.csv", header=T)
   exR_sg = read.csv("data/exR_sg_1L2F.csv", header=T)
   ...
   attach(sg_ns3)
   t.test(TotalEnergy,SgTotalEnergy,paired=FALSE,conf.level=0.95)
   cat("Difference in percentage is:",(2089.989 - 2052.243)/ 2089.989 * 100)
#+END_SRC
...
data:  TotalEnergy and SgTotalEnergy
t = 0.20853, df = 121.97, p-value = 0.8352
alternative hypothesis: true difference in means is not equal to 0
95 percent confidence interval:
 -320.5825  396.0754
sample estimates:
...
Difference in percentage is: 1.806038
#+end_example

**** Experiment 15: Flow 2, Links 3, bw 10Mbps, lat 10ms 
#+BEGIN_SRC bash :results silent :exports both 
#! /bin/bash
RANDOM=234
cd ~/simgrid/examples/msg/energy-link
for ((k=0; k<40; k++)); do
  ...
  bytesToSend=$((randVal * (10**6))) # traffic in MB between 20 and 100
  # run simgrid
   ./energy-link energy_consumption_3L_p.xml 2 ${bytesToSend} >> ~/internship/data/ex15_sg_3L2F.raw 2>&1
  # run ecofen
  # ./energy-link energy_consumption_3L_p.xml 2 ${bytesToSend} --NS3 --cfg=network/model:NS3 >> ~/internship/data/ex15_ns3_3L2FL.raw 2>&1
done
#+END_SRC 
***** Extract simgrid's data  
#+CALL: extract_sg_data_3(input_file="data/ex15_sg_3L2F.raw", output_file="data/ex15_sg_3L2F.csv",flow=2) 
***** Extract ecofen's data 
#+CALL: extract_ns3_data_5(input_file="data/ex15_ns3_3L2FL.raw", output_file="data/ex15_ns3_3L2FL.csv",flow=2)
***** Compare results
#+BEGIN_SRC R :results output graphics :file figures/ex15_energy_sg_vs_ns3.png :exports both :noweb yes

<<format_theme>>

   ex15_ns3 = read.csv("data/ex15_ns3_3L2FL.csv", header=T)
   ex15_sg = read.csv("data/ex15_sg_3L2F.csv", header=T)
   sg_ns3 = merge(ex15_ns3,ex15_sg, by=c("BytesSend"))
   attach(sg_ns3)
   ggplot(sg_ns3,aes(x=BytesSend))+
   geom_point(aes(y = TotalEnergy, colour = "ECOFEN"),size=1.3)+
   geom_point(aes(y = SgTotalEnergy, colour = "SimGrid"),size=1.3)+
...
#+RESULTS:
[[file:figures/ex15_energy_sg_vs_ns3.png]]
***** Statistical result
#+BEGIN_SRC R :results output :var validation=validation :exports both 
   ex15_ns3 = read.csv("data/ex15_ns3_3L2FL.csv", header=T)
   ex15_sg = read.csv("data/ex15_sg_3L2F.csv", header=T)
...
data:  TotalEnergy and SgTotalEnergy
t = -0.068237, df = 121.99, p-value = 0.9457
alternative hypothesis: true difference in means is not equal to 0
95 percent confidence interval:
 -1061.3625   990.6304
sample estimates:
...
**** Experiment 16: Run time and memory usage as a function of link size (bw 10Mbps, lat 10ms, data size 200 MBytes, flow 2, link range {1, 2, 4, 6, 8, 10})

#+BEGIN_SRC bash :results silent :exports both
#! /bin/bash
cd /home/bamersho/bamersho/simgrid/examples/msg/energy-link
randVal=200
bytesToSend=$((randVal * (10**6))) # traffic in MBytes
for ((k=0; k<11; k=k+2));
 do
    for ((j=0; j<7 j++));
    do
       # run simgrid with /usr/bin/time
       /usr/bin/time  -f "ElapsedTime %E ElapsedSec %e PeakMem %M AvgTotalMem %K" ./energy-link energy_consumption_${k}L_p.xml 2 ${bytesToSend} >> /home/bamersho/bamersho/data/perfLinkZ_sg_L${k}F2.raw 2>&1
       # run ecofen with /usr/bin/time
       /usr/bin/time  -f "ElapsedTime %E ElapsedSec %e PeakMem %M AvgTotalMem %K" ./energy-link energy_consumption_${k}L_p.xml 2 ${bytesToSend} --NS3 --cfg=network/model:NS3 >> /home/bamersho/bamersho/data/perfLinkZ_ns3_L${k}F2.raw 2>&1
     done
done
#+END_SRC
#+NAME: extract_perf_data
#+BEGIN_SRC python :results silent dir:~/internship :var version="" 
import sys

header = True

# data variable holds one line of data formated as follows 
# LinkSize, ElaspedTime, ElapsedSec, PeakMem, AvgTotalMem
data = []
...
PEAK_MEM_COL=5
AVG_TOTAL_MEM_COL=7
output_file_m = "data/perfLinkZ_"+version+"_F2.csv"  
for i in range(0, 12, 2):
    input_file_m = "data/perfLinkZ_"+version+"_L"+str(i)+"F2.raw" 
    with open(input_file_m) as fe:
        for line in fe:
           if line.startswith("ElapsedTime"): 
               list = line.split()
               data.append(str(i))
               data.append(list[ELAPSED_TIME_COL])
               data.append(list[ELAPSED_SEC_COL])
               data.append(list[PEAK_MEM_COL])
               data.append(list[AVG_TOTAL_MEM_COL])
           if len(data) == 5:
               with open(output_file_m,"a") as of: if header: # write the header part
                       h = "LinkSize, ElaspedTime"+version+", ElapsedSec"+version+", PeakMem"+version+", AvgTotalMem"+version
                       of.write("%s\n" % h )
                       header = False
                   of.write("%s\n" % ",".join(data))
                   del data[:]
#+END_SRC
***** Extract simgrid's data
#+CALL: extract_perf_data(version="sg") 
***** Extract ecofen's data
#+CALL: extract_perf_data(version="ns3")
***** Compare the run time performance
[[http://www.cookbook-r.com/Graphs/Plotting_means_and_error_bars_(ggplot2)/][How to plot error bars]]
#+BEGIN_SRC R :results output graphics :file figures/ex16_linksize_sg_vs_ns3.png :exports both :noweb yes

<<format_theme>>

   ex16_ns3 = read.csv("data/perfLinkZ_ns3_F2.csv", header=T)
   ex16_sg = read.csv("data/perfLinkZ_sg_F2.csv", header=T)
   ex16_sg$LinkSize=NULL
   sg_ns3 = cbind(ex16_ns3,ex16_sg)
   attach(sg_ns3)
   ns3_err = summarySE(sg_ns3, measurevar="ElapsedSecns3", groupvars="LinkSize")
   sg_err = summarySE(sg_ns3, measurevar="ElapsedSecsg", groupvars="LinkSize")
   pd = position_dodge(0.1)
   ggplot(sg_ns3,aes(x=LinkSize,y=ElapsedSecns3))+
   geom_line(aes(y = ElapsedSecns3, colour = "ECOFEN"),size=1.3)+
   geom_line(aes(y = ElapsedSecsg, colour = "SimGrid"),size=1.3)+
   geom_errorbar(aes(ymin=ElapsedSecns3-ns3_err$ci,ymax=ElapsedSecns3+ns3_err$ci),width=0.1,colour="brown")+
   geom_errorbar(aes(ymin=ElapsedSecsg-sg_err$ci,ymax=ElapsedSecsg+sg_err$ci),width=0.1,colour="brown")+
   ylab(label="Simulation time (seconds)")+
   xlab(label="Path length")+
   scale_colour_Publication()+
   scale_x_continuous(breaks = round(seq(min(sg_ns3$LinkSize), max(sg_ns3$LinkSize), by =1),1)) +
   theme_Publication()
#+END_SRC

#+RESULTS:
[[file:figures/ex16_linksize_sg_vs_ns3.png]]

***** Compare the peak memory usage performance
#+BEGIN_SRC R :results output graphics :file figures/ex16_linksizeM_sg_vs_ns3.png :exports both :noweb yes

<<format_theme>>

   ex16_ns3 = read.csv("data/perfLinkZ_ns3_F2.csv", header=T)
   ex16_ns3$PeakMemns3=ex16_ns3$PeakMemns3*(10^(-6))
   ex16_sg = read.csv("data/perfLinkZ_sg_F2.csv", header=T)
   ex16_sg$PeakMemsg=ex16_sg$PeakMemsg*(10^(-6))
   sg_ns3 = cbind(ex16_ns3,ex16_sg)
   attach(sg_ns3)
   ns3_err = summarySE(sg_ns3, measurevar="PeakMemns3", groupvars=c("LinkSize"))
   sg_err = summarySE(sg_ns3, measurevar="PeakMemsg", groupvars=c("LinkSize"))
   pd = position_dodge(0.1)
   ggplot(sg_ns3,aes(x=LinkSize))+
   geom_line(aes(y = PeakMemns3, colour = "ECOFEN"),size=1.3)+
   geom_line(aes(y = PeakMemsg, colour = "SimGrid"),size=1.3)+
   geom_errorbar(aes(ymin=PeakMemns3-ns3_err$ci,ymax=PeakMemns3+ns3_err$ci),width=0.1,colour="brown",position=pd)+
   geom_errorbar(aes(ymin=PeakMemsg-sg_err$ci,ymax=PeakMemsg+sg_err$ci),width=0.1,colour="brown",position=pd)+
   ylab(label="Peak Memory Usage in MB")+
   xlab(label="Path length")+
   scale_colour_Publication()+
   scale_x_continuous(breaks = round(seq(min(sg_ns3$LinkSize), max(sg_ns3$LinkSize), by =1),1)) +
   theme_Publication()
...
#+RESULTS:
[[file:figures/ex16_linksizeM_sg_vs_ns3.png]]

**** Experiment 17: Run time and memory usage as a function of data size (bw 10Mbps, lat 10ms, data size range [10,50]MBytes, flow 2, link 2)
#+BEGIN_SRC bash :results silent :exports both
#! /bin/bash
cd /home/bamersho/bamersho/simgrid/examples/msg/energy-link
RANDOM=234
...
randVal=$(((RANDOM % 50) + 10))
bytesToSend=$((randVal * (10**6))) # traffic in MBytes

    for ((k=0; k<10; k++));
    do
       # run simgrid with /usr/bin/time
       /usr/bin/time -f "ElapsedTime %E ElapsedSec %e PeakMem %M AvgTotMem %K" ./energy-link energy_consumption_2L_p.xml 2 ${bytesToSend} >> /home/bamersho/bamersho/data/perfDat_sg_T${randVal}L2F2.raw 2>&1
       # run ecofen with /usr/bin/time
       /usr/bin/time -f "ElapsedTime %E ElapsedSec %e PeakMem %M AvgTotMem %K" ./energy-link energy_consumption_2L_p.xml 2 ${bytesToSend} --NS3 --cfg=network/model:NS3 >> /home/bamersho/bamersho/data/perfDat_ns3_T${randVal}L2F2.raw 2>&1
    done
done
#+END_SRC
***** Extract simgrid's data
***** Extract ecofen's data
**** Experiment 18: checking simulated time vs bandwidth (1 link, 2 flow, 100 MBytes data size, bw range (10 to 500 with increment of 10)
#+BEGIN_SRC bash :results silent :exports both 
#! /bin/bash
cd ~/simgrid/examples/msg/energy-link
randVal=100
bytesToSend=$((randVal * (10**6))) # traffic in MBytes
for ((k=10; k<510; k=k+10)); do
  cat energy_consumption_1L_p.xml |sed -e "s/\$B/${k}/"  > t_energy_consumption_1L_p.xml 
  # run simgrid
   ./energy-link t_energy_consumption_1L_p.xml 2 ${bytesToSend} >> ~/internship/data/ex18_sg_bw_tim.raw 2>&1
  # run ecofen
   ./energy-link t_energy_consumption_1L_p.xml 2 ${bytesToSend} --NS3 --cfg=network/model:NS3 >> ~/internship/data/ex18_ns3_bw_tim.raw 2>&1
done
#+END_SRC
***** Extract simgrid's data
#+NAME: extract_sg_bw_time_data 
#+BEGIN_SRC python :results silent dir:~/internship :var input_file="" :var output_file=""  
import sys
...
# data variable holds one line of data formated as follows 
# Bandwidth, SgAveragePower, SgTransferTime, SgTotalEnergy,
data = []
    ...
    for line in fe:
       ...
       if len(data) == 4 and done is True:
           with open(output_file,"a") as of:
               ...
               del data[:]
#+END_SRC
#+CALL: extract_sg_bw_time_data(input_file="data/ex18_sg_bw_tim.raw", output_file="data/ex18_sg_bw_tim.csv") 
***** Extract ecofen's data
#+NAME: extract_ns3_bw_time_data
#+BEGIN_SRC python :results silent dir:~/internship :var input_file="" :var output_file=""  
import sys
...
# data variable holds one line of data formated as follows 
# Bandwidth,AveragePowerNs3,TotalTimeTakenNs3,TotalEnergyNs3
data = []
done = False
traffic = 0.0
    ...
    for line in fe:
       ...
       if len(data) == 4 and done is True:
           with open(output_file,"a") as of:
               ...
               cur_time = 0
#+END_SRC
#+CALL: extract_ns3_bw_time_data(input_file="data/ex18_ns3_bw_tim.raw", output_file="data/ex18_ns3_bw_tim.csv") 
***** Compare bandwidth vs time of simgrid and ecofen
#+BEGIN_SRC R :results output graphics :file figures/ex18_bw_time_sg_vs_ns3.png :exports both :noweb yes
<<format_theme>>
   ex18_ns3 = read.csv("data/ex18_ns3_bw_tim.csv", header=T)
   ex18_sg = read.csv("data/ex18_sg_bw_tim.csv", header=T)
   sg_ns3 = merge(ex18_ns3,ex18_sg, by=c("Bandwidth"))
   attach(sg_ns3)
   ggplot(sg_ns3,aes(x=Bandwidth))+
   geom_point(aes(y = TotalTimeTakenNs3, colour = "ECOFEN"),size=1.3)+
   geom_point(aes(y = SgTransferTime, colour = "SimGrid"),size=1.3)+
   ylab(label="Data transfer time (in seconds)")+
   xlab(label="Bandwidth (in Mbps)")+
   scale_colour_Publication()+
   theme_Publication()
#+END_SRC

#+RESULTS:
[[file:figures/ex18_bw_time_sg_vs_ns3.png]]
**** Experiment 18-1: checking simulated time vs bandwidth (1 link, 1 flow, 100 MBytes data size, bw range (10 to 500 with increment of 10)
#+BEGIN_SRC bash :results silent :exports both 
#! /bin/bash
cd ~/simgrid/examples/msg/energy-link
randVal=100
...
for ((k=10; k<510; k=k+10)); do
  cat energy_consumption_1L_p.xml |sed -e "s/\$B/${k}/"  > t_energy_consumption_1L_p.xml 
  # run simgrid
   ./energy-link t_energy_consumption_1L_p.xml 1 ${bytesToSend} >> ~/internship/data/ex18_sg_bw_tim_F1.raw 2>&1
  # run ecofen
   ./energy-link t_energy_consumption_1L_p.xml 1 ${bytesToSend} --NS3 --cfg=network/model:NS3 >> ~/internship/data/ex18_ns3_bw_tim_F1.raw 2>&1
done
#+END_SRC
***** Extract SimGrid data
#+CALL: extract_sg_bw_time_data(input_file="data/ex18_sg_bw_tim_F1.raw", output_file="data/ex18_sg_bw_tim_F1.csv") 
***** Extract ECOFEN's data
#+CALL: extract_ns3_bw_time_data(input_file="data/ex18_ns3_bw_tim_F1.raw", output_file="data/ex18_ns3_bw_tim_F1.csv")
***** Compare bandwidth vs time of simgrid and ecofen
#+BEGIN_SRC R :results output graphics :file figures/ex18_bw_time_sg_vs_ns3_F1.png :exports both :noweb yes
<<format_theme>>
   ex18_ns3 = read.csv("data/ex18_ns3_bw_tim_F1.csv", header=T)
   ex18_sg = read.csv("data/ex18_sg_bw_tim_F1.csv", header=T)
   sg_ns3 = merge(ex18_ns3,ex18_sg, by=c("Bandwidth"))
   attach(sg_ns3)
   ggplot(sg_ns3,aes(x=Bandwidth))+
   geom_point(aes(y = TotalTimeTakenNs3, colour = "ECOFEN"),size=1.3)+
...
#+RESULTS:
[[file:figures/ex18_bw_time_sg_vs_ns3_F1.png]]

**** Experiment 19: checking simulated time vs data_size (1 link, 2 flow, bw 50Mbps, data size, data size range (10 to 200 with increment of 10)
#+BEGIN_SRC bash :results silent :exports both 
#! /bin/bash
cd ~/simgrid/examples/msg/energy-link
for ((k=5; k<200; k=k+10)); do
  bytesToSend=$((k * (10**6))) # traffic in MBytes
  # run simgrid
   ./energy-link energy_consumption_1L_p.xml 2 ${bytesToSend} >> ~/internship/data/ex19-1_sg_dsize_tim.raw 2>&1
  # run ecofen
   ./energy-link energy_consumption_1L_p.xml 2 ${bytesToSend} --NS3 --cfg=network/model:NS3 >> ~/internship/data/ex19-1_ns3_dsize_tim.raw 2>&1
done
#+END_SRC
***** Extract simgrid's data  
#+CALL: extract_sg_data_3(input_file="data/ex19_sg_dsize_tim.raw", output_file="data/ex19_sg_dsize_tim.csv",flow=2) 
***** Extract ecofen's data 
#+CALL: extract_ns3_data_5(input_file="data/ex19_ns3_dsize_tim.raw", output_file="data/ex19_ns3_dsize_tim.csv",flow=2)
***** Compare results
#+BEGIN_SRC R :results output :var validation=validation :exports both 
   ex19_ns3 = read.csv("data/ex19_ns3_dsize_tim.csv", header=T)
   ex19_sg = read.csv("data/ex19_sg_dsize_tim.csv", header=T)
   sg_ns3 = merge(ex19_ns3,ex19_sg, by=c("BytesSend"))
   attach(sg_ns3)
   t.test(TotalTimeTaken,SgTransferTime,paired=FALSE,conf.level=0.95)
#+END_SRC
...
data:  TotalTimeTaken and SgTransferTime
t = -0.01859, df = 37.998, p-value = 0.9853
alternative hypothesis: true difference in means is not equal to 0
95 percent confidence interval:
 -12.79517  12.56231
sample estimates:
...
**** Experiment 20: checking simulated time vs data_size using experiment 12's data (1 link, 2 flow, bw 10Mbps, data size, data size range (10 to 200) 
***** Compare results
#+BEGIN_SRC R :results output :var validation=validation :exports both 
   ex19_ns3 = read.csv("data/ex19-1_ns3_dsize_tim.csv", header=T)
   ex19_sg = read.csv("data/ex19-1_sg_dsize_tim.csv", header=T)
   sg_ns3 = merge(ex19_ns3,ex19_sg, by=c("BytesSend"))
   attach(sg_ns3)
   t.test(TotalTimeTaken,SgTransferTime,paired=FALSE,conf.level=0.95)
#+END_SRC
...
data:  TotalTimeTaken and SgTransferTime
t = -0.036534, df = 37.998, p-value = 0.971
alternative hypothesis: true difference in means is not equal to 0
95 percent confidence interval:
 -64.53773  62.24962
sample estimates:
...
**** Experiment 21: Run time and memory usage as a function of data size (bw 10 Mbps, lat 10ms, data size [50,500] Mbytes, flow 2, link size 1)
#+NAME: extract_perf_data1
#+BEGIN_SRC python :results silent dir:~/internship :var version="" 
import sys
...
# data variable holds one line of data formated as follows 
# DataSize, ElaspedTime, ElapsedSec, PeakMem
data = []

ELAPSED_TIME_COL=1
ELAPSED_SEC_COL=3
PEAK_MEM_COL=5
output_file_m = "data/perfDatY_"+version+"_L2F2.csv"  
#dataRange=[101,105,106,10,16,25,32,61,71,72,81,85,86,88,90,92,97,99]
#dataRange=[10,16,21,31,32,35,36,38,40,42,47,51,56]
dataRange=[111,112,122,125,126,145,151,156,178,191,232,237,261,289,346,365,467,471,476,50,530]
for i in dataRange:
    input_file_m = "data/perfDatY_"+version+"_T"+str(i)+"L2F2.raw" 
    with open(input_file_m) as fe:
        for line in fe:
           if line.startswith("ElapsedTime"): 
               list = line.split()
               ...
               data.append(list[PEAK_MEM_COL])
           if len(data) == 4:
               with open(output_file_m,"a") as of:
                   if header: # write the header part
                       h = "DataSize, ElaspedTime"+version+", ElapsedSec"+version+", PeakMem"+version
                       of.write("%s\n" % h )
                       header = False
                   of.write("%s\n" % ",".join(data))
                   del data[:]
#+END_SRC
***** Extract simgrid's data
#+CALL: extract_perf_data1(version="sg") 
***** Extract ecofen's data
#+CALL: extract_perf_data1(version="ns3")
***** Compare the run time performance
[[http://www.cookbook-r.com/Graphs/Plotting_means_and_error_bars_(ggplot2)/][How to plot error bars]]
#+BEGIN_SRC R :results output graphics :file figures/ex21_datasize_sg_vs_ns3.png :exports both :noweb yes

<<format_theme>>

   ex21_ns3 = read.csv("data/perfDatY_ns3_L2F2.csv", header=T)
   ex21_sg = read.csv("data/perfDatY_sg_L2F2.csv", header=T)
   ex21_sg$DataSize=NULL
   sg_ns3 = cbind(ex21_ns3,ex21_sg)
   attach(sg_ns3)
   ns3_err = summarySE(sg_ns3, measurevar="ElapsedSecns3", groupvars="DataSize")
   sg_err = summarySE(sg_ns3, measurevar="ElapsedSecsg", groupvars="DataSize")
   pd = position_dodge(0.1)
   ggplot(sg_ns3,aes(x=DataSize))+
   geom_line(aes(y = ElapsedSecns3, colour = "ECOFEN"),size=1.3)+
   geom_line(aes(y = ElapsedSecsg, colour = "SimGrid"),size=1.3)+
   geom_errorbar(aes(ymin=ElapsedSecns3-ns3_err$ci,ymax=ElapsedSecns3+ns3_err$ci),width=0.1,colour="brown")+
   geom_errorbar(aes(ymin=ElapsedSecsg-sg_err$ci,ymax=ElapsedSecsg+sg_err$ci),width=0.1,colour="brown")+
   ylab(label="Simulation time (seconds)")+
   xlab(label="Data  size (Mbytes)")+
   scale_colour_Publication()+
   theme_Publication()
#+END_SRC

#+RESULTS:
[[file:figures/ex21_datasize_sg_vs_ns3.png]]

***** Compare the peak memory usage performance
#+BEGIN_SRC R :results output graphics :file figures/ex21_DataSizeM_sg_vs_ns3.png :exports both :noweb yes

<<format_theme>>

   ex21_ns3 = read.csv("data/perfDatY_ns3_L2F2.csv", header=T)
   ex21_ns3$PeakMemns3=ex21_ns3$PeakMemns3*(10^(-6))
   ex21_sg = read.csv("data/perfDatY_sg_L2F2.csv", header=T)
   ex21_sg$DataSize=NULL
   ex21_sg$PeakMemsg=ex21_sg$PeakMemsg*(10^(-6))
   sg_ns3 = cbind(ex21_ns3,ex21_sg)
   attach(sg_ns3)
   ns3_err = summarySE(sg_ns3, measurevar="PeakMemns3", groupvars=c("DataSize"))
   sg_err = summarySE(sg_ns3, measurevar="PeakMemsg", groupvars=c("DataSize"))
   pd = position_dodge(0.1)
   ggplot(sg_ns3,aes(x=DataSize))+
   geom_line(aes(y = PeakMemns3, colour = "ECOFEN"),size=1.3)+
   geom_line(aes(y = PeakMemsg, colour = "SimGrid"),size=1.3)+
   ...
   geom_errorbar(aes(ymin=PeakMemsg-sg_err$ci,ymax=PeakMemsg+sg_err$ci),width=0.1,colour="brown",position=pd)+
   ylab(label="Peak Memory Usage in MB")+
   xlab(label="DataSize (Mbytes)")+
   scale_colour_Publication()+
   theme_Publication()
#+END_SRC

#+RESULTS:
[[file:figures/ex21_DataSizeM_sg_vs_ns3.png]]
** Week 3 June
**** Result presentation as Mean Log Error and Max Log Error format 
#+BEGIN_SRC R :results output graphics :file figures/logerror_sg_vs_ns3.png :exports both :noweb yes
<<format_theme>>
   # 1L1F
   ex11_ns3 = read.csv("data/ex11_ns3_1L1F.csv", header=T)
   ex11_sg = read.csv("data/ex11_sg_1L1F.csv", header=T)
   ex11 = merge(ex11_ns3,ex11_sg, by=c("BytesSend"))
   ex11M=mean(log10(ex11$TotalEnergy)-log10(ex11$SgTotalEnergy))
   ex11P=((exp(mean(log10(ex11$SgTotalEnergy)-log10(ex11$TotalEnergy)))) - 1)*100
  #t.test(ex11$TotalEnergy,ex11$SgTotalEnergy,paired=FALSE,conf.level=0.95)
   # 1L2F
   ex12_ns3 = read.csv("data/ex12_ns3_1L2FL.csv", header=T)
   ex12_sg = read.csv("data/ex12_sg_1L2F.csv", header=T)
   ex12 = merge(ex12_ns3,ex12_sg, by=c("BytesSend"))
   ex12M=mean(log10(ex12$TotalEnergy)-log10(ex12$SgTotalEnergy))
   ex12P=((exp(mean(log10(ex12$SgTotalEnergy)-log10(ex12$TotalEnergy)))) - 1)*100
  #t.test(ex12$TotalEnergy,ex12$SgTotalEnergy,paired=FALSE,conf.level=0.95)
   # 1L4F
   ex10_ns3 = read.csv("data/ex10_ns3_1L4F.csv", header=T)
   ex10_sg = read.csv("data/ex10_sg_1L4F.csv", header=T)
   ex10 = merge(ex10_ns3,ex10_sg,by=c("BytesSend"))
   ex10M=mean(log10(ex10$TotalEnergy)-log10(ex10$SgTotalEnergy))
   ex10P=((exp(mean(log10(ex10$SgTotalEnergy)-log10(ex10$TotalEnergy)))) - 1)*100
  #t.test(ex10$TotalEnergy,ex10$SgTotalEnergy,paired=FALSE,conf.level=0.95)
   # 3L1F
   ex13_ns3 = read.csv("data/ex13_ns3_3L1FL.csv", header=T)
   ex13_sg = read.csv("data/ex13_sg_3L1F.csv", header=T)
   ex13 = merge(ex13_ns3,ex13_sg, by=c("BytesSend"))
   ex13M=mean(log10(ex13$TotalEnergy)-log10(ex13$SgTotalEnergy))
   ex13P=((exp(mean(log10(ex13$SgTotalEnergy)-log10(ex13$TotalEnergy)))) - 1)*100
  #t.test(ex13$TotalEnergy,ex13$SgTotalEnergy,paired=FALSE,conf.level=0.95)
   # 3L2F
   ex15_ns3 = read.csv("data/ex15_ns3_3L2FL.csv", header=T)
   ex15_sg = read.csv("data/ex15_sg_3L2F.csv", header=T)
   ex15 = merge(ex15_ns3,ex15_sg, by=c("BytesSend"))
   ex15M=mean(log10(ex15$TotalEnergy)-log10(ex15$SgTotalEnergy))
   ex15P=((exp(mean(log10(ex15$SgTotalEnergy)-log10(ex15$TotalEnergy)))) - 1)*100
  #t.test(ex15$TotalEnergy,ex15$SgTotalEnergy,paired=FALSE,conf.level=0.95)

  logErr = data.frame(Scenarios=c("1L1F","1L2F","1L4F","3L1F","3L2F"),
                      logError=c(ex11M,ex12M,ex10M,ex13M,ex15M))
   attach(logErr)
   ggplot(logErr,aes(x=Scenarios,y=logError))+
   geom_point()+
   ylab(label="Mean Log Error")+
   xlab(label="Scenarios")+
   scale_colour_Publication()+
   theme_Publication()
#+END_SRC

#+RESULTS:
[[file:figures/logerror_sg_vs_ns3.png]]

** Week 2 August
*** Italy ISP network topology 
#+BEGIN_SRC python :results output
import sys

def printnodes(n1,n2,zn):
    print ("""
 
      <zone id="{2}" routing="Full">
      <router id="b{0}"/>
		  <router id="b{1}"/>
		  <router id="m{0}"/>
		  <router id="m{1}"/>
		  
		  <link id="b{0}-{1}" bandwidth="20Mbps" latency="10ms" />
		  <link id="m{0}b{0}" bandwidth="20Mbps" latency="10ms" />
		  <link id="m{0}b{1}" bandwidth="20Mbps" latency="10ms" />
		  <link id="m{1}b{0}" bandwidth="20Mbps" latency="10ms" />
		  <link id="m{1}b{1}" bandwidth="20Mbps" latency="10ms" />
		  
		  <route src="b{0}" dst="b{1}">
			<link_ctn id="b{0}-{1}" />
		  </route>	
		  <route src="m{0}" dst="b{1}">
			<link_ctn id="m{0}b{0}" />
		  </route>	  
		   <route src="m{0}" dst="b{1}">
			<link_ctn id="m{0}b{1}" />
		  </route>	
		  <route src="m{1}" dst="b{0}">
			<link_ctn id="m{1}b{0}" />
		  </route>	  
		   <route src="m{1}" dst="b{1}">
			<link_ctn id="m{1}b{1}" />
		  </route>	
		  
		</zone>
		
		<link id="b{0}c1" bandwidth="20Mbps" latency="10ms" />
		<link id="b{0}c3" bandwidth="20Mbps" latency="10ms" />
		<link id="b{1}c2" bandwidth="20Mbps" latency="10ms" />
		<link id="b{1}c4" bandwidth="20Mbps" latency="10ms" />
		
		<zoneRoute src="{2}" dst="core" gw_src="b{0}" gw_dst="c1">
			<link_ctn id="b{0}c1" />
		</zoneRoute>		
		<zoneRoute src="{2}" dst="core" gw_src="b{0}" gw_dst="c3">
			<link_ctn id="b{0}c3" />
		</zoneRoute>
		<zoneRoute src="{2}" dst="core" gw_src="b{1}" gw_dst="c2">
			<link_ctn id="b{1}c2" />
		</zoneRoute>		
		<zoneRoute src="{2}" dst="core" gw_src="b{1}" gw_dst="c4">
			<link_ctn id="b{1}c4" />
		</zoneRoute>
    """.format(n1,n2,zn))
...
def access_nodes(srcZone,dstZone,count,cnt):
    return """
      <zone id="access{14}" routing="Full">
			<router id="a{2}" />
			<router id="a{3}" />
			<router id="a{4}" />
			<router id="a{5}" />
      ...
      <router id="a{7}" />
			...
			<router id="a{10}" />
			<router id="a{11}" />
			<host id="h{2}" speed="100.0Mf,50.0Mf,20.0Mf" pstate="0">
				<prop id="watt_off" value="0" />
			</host>
			<host id="h{3}" speed="100.0Mf,50.0Mf,20.0Mf" pstate="0">
				<prop id="watt_off" value="0" />
			</host>
			<host id="h{4}" speed="100.0Mf,50.0Mf,20.0Mf" pstate="0">
				<prop id="watt_off" value="0" />
			</host>
			<host id="h{5}" speed="100.0Mf,50.0Mf,20.0Mf" pstate="0">
				<prop id="watt_off" value="0" />
			</host>
			<host id="h{6}" speed="100.0Mf,50.0Mf,20.0Mf" pstate="0">
				<prop id="watt_off" value="0" />
			</host>
      <host id="h{7}" speed="100.0Mf,50.0Mf,20.0Mf" pstate="0">
				<prop id="watt_off" value="0" />
			</host>
			<host id="h{8}" speed="100.0Mf,50.0Mf,20.0Mf" pstate="0">
				<prop id="watt_off" value="0" />
			</host>
			<host id="h{9}" speed="100.0Mf,50.0Mf,20.0Mf" pstate="0">
				<prop id="watt_off" value="0" />
			</host>
			<host id="h{10}" speed="100.0Mf,50.0Mf,20.0Mf" pstate="0">
				<prop id="watt_off" value="0" />
			</host>
			<host id="h{11}" speed="100.0Mf,50.0Mf,20.0Mf" pstate="0">
				<prop id="watt_off" value="0" />
			</host>

			<link id="h{2}a{2}" bandwidth="10Mbps" latency="10ms">
				<prop id="watt_range" value="10.3581:10.7479" />
			</link>
			<link id="h{3}a{3}" bandwidth="10Mbps" latency="10ms">
				<prop id="watt_range" value="10.3581:10.7479" />
			</link>
			<link id="h{4}a{4}" bandwidth="10Mbps" latency="10ms">
				<prop id="watt_range" value="10.3581:10.7479" />
			</link>
			<link id="h{5}a{5}" bandwidth="10Mbps" latency="10ms">
				<prop id="watt_range" value="10.3581:10.7479" />
			</link>
			<link id="h{6}a{6}" bandwidth="10Mbps" latency="10ms">
				<prop id="watt_range" value="10.3581:10.7479" />
			</link>
      <link id="h{7}a{7}" bandwidth="10Mbps" latency="10ms">
				<prop id="watt_range" value="10.3581:10.7479" />
			</link>
			<link id="h{8}a{8}" bandwidth="10Mbps" latency="10ms">
				<prop id="watt_range" value="10.3581:10.7479" />
			</link>
			<link id="h{9}a{9}" bandwidth="10Mbps" latency="10ms">
				<prop id="watt_range" value="10.3581:10.7479" />
			</link>
			<link id="h{10}a{10}" bandwidth="10Mbps" latency="10ms">
				<prop id="watt_range" value="10.3581:10.7479" />
			</link>
			<link id="h{11}a{11}" bandwidth="10Mbps" latency="10ms">
				<prop id="watt_range" value="10.3581:10.7479" />
			</link>

			<route src="h{2}" dst="a{2}">
				<link_ctn id="h{2}a{2}" />
			</route>
			<route src="h{3}" dst="a{3}">
				<link_ctn id="h{3}a{3}" />
			</route>
			<route src="h{4}" dst="a{4}">
				<link_ctn id="h{4}a{4}" />
			</route>
			<route src="h{5}" dst="a{5}">
				<link_ctn id="h{5}a{5}" />
			</route>
			<route src="h{6}" dst="a{6}">
				<link_ctn id="h{6}a{6}" />
			</route>
      <route src="h{7}" dst="a{2}">
				<link_ctn id="h{7}a{7}" />
			</route>
			<route src="h{8}" dst="a{8}">
				<link_ctn id="h{8}a{8}" />
			</route>
			<route src="h{9}" dst="a{9}">
				<link_ctn id="h{9}a{9}" />
			</route>
			<route src="h{10}" dst="a{10}">
				<link_ctn id="h{10}a{10}" />
			</route>
			<route src="h{11}" dst="a{11}">
				<link_ctn id="h{11}a{11}" />
			</route>
		</zone>
		
		<link id="a{2}m{12}" bandwidth="20Mbps" latency="10ms">
				<prop id="watt_range" value="10.3581:10.7479" />
		</link>
		<link id="a{2}m{13}" bandwidth="20Mbps" latency="10ms">
				<prop id="watt_range" value="10.3581:10.7479" />
		</link>
		<link id="a{3}m{12}" bandwidth="20Mbps" latency="10ms">
				<prop id="watt_range" value="10.3581:10.7479" />
		</link>
		<link id="a{3}m{13}" bandwidth="20Mbps" latency="10ms">
				<prop id="watt_range" value="10.3581:10.7479" />
		</link>
		<link id="a{4}m{12}" bandwidth="20Mbps" latency="10ms">
				<prop id="watt_range" value="10.3581:10.7479" />
		</link>
		<link id="a{4}m{13}" bandwidth="20Mbps" latency="10ms">
				<prop id="watt_range" value="10.3581:10.7479" />
		</link>
		<link id="a{5}m{12}" bandwidth="20Mbps" latency="10ms">
				<prop id="watt_range" value="10.3581:10.7479" />
		</link>
		<link id="a{5}m{13}" bandwidth="20Mbps" latency="10ms">
				<prop id="watt_range" value="10.3581:10.7479" />
		</link>
		<link id="a{6}m{12}" bandwidth="20Mbps" latency="10ms">
				<prop id="watt_range" value="10.3581:10.7479" />
		</link>
		<link id="a{6}m{13}" bandwidth="20Mbps" latency="10ms">
				<prop id="watt_range" value="10.3581:10.7479" />
		</link>
    <link id="a{7}m{12}" bandwidth="20Mbps" latency="10ms">
				<prop id="watt_range" value="10.3581:10.7479" />
		</link>
		<link id="a{7}m{13}" bandwidth="20Mbps" latency="10ms">
				<prop id="watt_range" value="10.3581:10.7479" />
		</link>
		<link id="a{8}m{12}" bandwidth="20Mbps" latency="10ms">
				<prop id="watt_range" value="10.3581:10.7479" />
		</link>
		<link id="a{8}m{13}" bandwidth="20Mbps" latency="10ms">
				<prop id="watt_range" value="10.3581:10.7479" />
		</link>
		<link id="a{9}m{12}" bandwidth="20Mbps" latency="10ms">
				<prop id="watt_range" value="10.3581:10.7479" />
		</link>
		<link id="a{9}m{13}" bandwidth="20Mbps" latency="10ms">
				<prop id="watt_range" value="10.3581:10.7479" />
		</link>
		<link id="a{10}m{12}" bandwidth="20Mbps" latency="10ms">
				<prop id="watt_range" value="10.3581:10.7479" />
		</link>
		<link id="a{10}m{13}" bandwidth="20Mbps" latency="10ms">
				<prop id="watt_range" value="10.3581:10.7479" />
		</link>
		<link id="a{11}m{12}" bandwidth="20Mbps" latency="10ms">
				<prop id="watt_range" value="10.3581:10.7479" />
		</link>
		<link id="a{11}m{13}" bandwidth="20Mbps" latency="10ms">
				<prop id="watt_range" value="10.3581:10.7479" />
		</link>
		
		<zoneRoute src="{0}" dst="{1}" gw_src="a{2}" gw_dst="m{12}">
			<link_ctn id="a{2}m{12}" />
		</zoneRoute>
		<zoneRoute src="{0}" dst="{1}" gw_src="a{2}" gw_dst="m{13}">
			<link_ctn id="a{2}m{13}" />
		</zoneRoute>
		<zoneRoute src="{0}" dst="{1}" gw_src="a{3}" gw_dst="m{12}">
			<link_ctn id="a{3}m{12}" />
		</zoneRoute>
		<zoneRoute src="{0}" dst="{1}" gw_src="a{3}" gw_dst="m{13}">
			<link_ctn id="a{3}m{13}" />
		</zoneRoute>
		<zoneRoute src="{0}" dst="{1}" gw_src="a{4}" gw_dst="m{12}">
			<link_ctn id="a{4}m{12}" />
		</zoneRoute>
		<zoneRoute src="{0}" dst="{1}" gw_src="a{4}" gw_dst="m{13}">
			<link_ctn id="a{4}m{13}" />
		</zoneRoute>
		<zoneRoute src="{0}" dst="{1}" gw_src="a{5}" gw_dst="m{12}">
			<link_ctn id="a{5}m{12}" />
		</zoneRoute>
		<zoneRoute src="{0}" dst="{1}" gw_src="a{5}" gw_dst="m{13}">
			<link_ctn id="a{5}m{13}" />
		</zoneRoute>
		<zoneRoute src="{0}" dst="{1}" gw_src="a{6}" gw_dst="m{12}">
			<link_ctn id="a{6}m{12}" />
		</zoneRoute>
		<zoneRoute src="{0}" dst="{1}" gw_src="a{6}" gw_dst="m{13}">
			<link_ctn id="a{6}m{13}" />
		</zoneRoute>
    <zoneRoute src="{0}" dst="{1}" gw_src="a{7}" gw_dst="m{12}">
			<link_ctn id="a{7}m{12}" />
		</zoneRoute>
		<zoneRoute src="{0}" dst="{1}" gw_src="a{7}" gw_dst="m{13}">
			<link_ctn id="a{7}m{13}" />
		</zoneRoute>
		<zoneRoute src="{0}" dst="{1}" gw_src="a{8}" gw_dst="m{12}">
			<link_ctn id="a{8}m{12}" />
		</zoneRoute>
		<zoneRoute src="{0}" dst="{1}" gw_src="a{8}" gw_dst="m{13}">
			<link_ctn id="a{8}m{13}" />
		</zoneRoute>
		<zoneRoute src="{0}" dst="{1}" gw_src="a{9}" gw_dst="m{12}">
			<link_ctn id="a{9}m{12}" />
		</zoneRoute>
		<zoneRoute src="{0}" dst="{1}" gw_src="a{9}" gw_dst="m{13}">
			<link_ctn id="a{9}m{13}" />
		</zoneRoute>
		<zoneRoute src="{0}" dst="{1}" gw_src="a{10}" gw_dst="m{12}">
			<link_ctn id="a{10}m{12}" />
		</zoneRoute>
		<zoneRoute src="{0}" dst="{1}" gw_src="a{10}" gw_dst="m{13}">
			<link_ctn id="a{10}m{13}" />
		</zoneRoute>
		<zoneRoute src="{0}" dst="{1}" gw_src="a{11}" gw_dst="m{12}">
			<link_ctn id="a{11}m{12}" />
		</zoneRoute>
		<zoneRoute src="{0}" dst="{1}" gw_src="a{11}" gw_dst="m{13}">
			<link_ctn id="a{11}m{13}" />
		</zoneRoute>
           """.format(srcZone,dstZone,count,count+1,count+2,count+3,count+4,count+5,count+6,count+7,count+8,count+9,(2*cnt)-1,2*cnt,cnt)

nodes=""
c = 1
for node in range(1,27):
   nodes += access_nodes("access"+str(node),"bm"+str(node),c,node)
   c+=10
...
def feeder_nodes(count):
   return """<zoneRoute src="feeder{0}" dst="bm{0}" gw_src="feeder{0}_gw"
				gw_dst="bm{0}_gw">
				<link_ctn id="feeder{0}_bm{0}_l" />
			</zoneRoute>\n""".format(count)
nodes=""
for node in range(1,27):
    nodes+=feeder_nodes(node) 
