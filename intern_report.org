...
j=$((header + i))
  for ((k=20; k<=60; k++)); do
 ./waf --run "scratch/validate_ecofen_modified --packetSize=${i} --rngRun=${k}" >> ~/internship/data/eco_val_modified1_${j}.dat 2>&1
   done
 done
#+END_SRC
...
#+CALL: extract_data(input_file="data/eco_val_modified1_100.dat", output_file="data/eco_val_modified1_100.csv",size=100) 
#+CALL: extract_data(input_file="data/eco_val_modified1_576.dat", output_file="data/eco_val_modified1_576.csv",size=576) 
#+CALL: extract_data(input_file="data/eco_val_modified1_1000.dat", output_file="data/eco_val_modified1_1000.csv",size=1000) 
#+CALL: extract_data(input_file="data/eco_val_modified1_1500.dat", output_file="data/eco_val_modified1_1500.csv",size=1500) 
...
library(ggplot2)
val_100 = read.csv("data/eco_val_modified1_100.csv", header=T)
val_576 = read.csv("data/eco_val_modified1_576.csv", header=T)
val_1000 = read.csv("data/eco_val_modified1_1000.csv", header=T)
val_1500 = read.csv("data/eco_val_modified1_1500.csv", header=T)
all_data = rbind(val_100,val_576,val_1000,val_1500)
attach(all_data)
ggplot(all_data,aes(x=Throughput))+
   geom_point(aes(y = Cons_Node0, colour = "Simulated Node-(0 or 2)"))+
...
- Running [[file:ns3/validate_ecofen_complete.cc][new NS-3 script]] as shown in the shell code block below produces files: [[file:data/eco_val1_complete_576.dat]] and
  [[file:data/eco_val_complete1_1500.dat]] both of theses file are for the receiving end, and [[file:data/eco_val_complete1_1000.dat]] is used for the sending
  end. 
#+BEGIN_SRC bash :results output :exports both
#! /bin/bash
cd ~/ns3workspace/source/ns-3.26
...
j=$((header + i))
  for ((k=1; k<=40; k++)); do
 ./waf --run "scratch/validate_ecofen_complete --packetSize=${i} --rngRun=${k}" >> ~/internship/data/eco_val_complete1_${j}.dat 2>&1
   done
 done
#+END_SRC
          ...
          with open(output_file,"a") as of:
              if len(data) == 9:
                  if header: # writes the header and the idle consumption
                      ...
                      del data[:]
#+END_SRC
#+CALL: extract_complete_data(input_file="data/eco_val_complete1_576.dat", output_file="data/eco_val_complete1_576.csv",size=576) 
#+CALL: extract_complete_data(input_file="data/eco_val_complete1_1000.dat", output_file="data/eco_val_complete1_1000.csv",size=1000) 
#+CALL: extract_complete_data(input_file="data/eco_val_complete1_1500.dat", output_file="data/eco_val_complete1_1500.csv",size=1500) 
...
library(ggplot2)
val_576 = read.csv("data/eco_val_complete1_576.csv", header=T)
val_1500 = read.csv("data/eco_val_complete1_1500.csv", header=T)
...
library(ggplot2)
val_1000 = read.csv("data/eco_val_complete1_1000.csv", header=T)
attach(val_1000)
...
|-------------+-----------+--------------------------+------------------+-------------------+---------|
|         576 | Receiving | [-0.06544964,0.03207116] |         10.75045 |          10.76714 |  0.4977 |
|        1500 | Receiving | [-0.01301081,0.08233968] |         10.75565 |          10.72098 |  0.1517 |
|        1000 | Sending   | [-0.08695043,0.04715759] |         10.72351 |          10.74340 |  0.5565 |

- From all of the p-values shown above, which are less than 0.05 for the 95% confidence level, tells us that the null hypothesis that the difference
  in mean is 0 is rejected in all of the three cases, which means the difference is true positive or it is not by chance.
