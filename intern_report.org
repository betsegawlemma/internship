...
- The p-value tells us that the null hypothesis that the difference in mean is 0 is not rejected. Further more, from the 95% confidence interval we
  can see that 0 is in the interval which means the difference is by chance or it is false-positive.    
#+BEGIN_SRC R :results output :var validation=validation :exports both 
attach(validation)
...
*** Questions 
- While writing the background section, I claimed that the reason we want to manage energy consumption is because currently IT equipment are not
  energy proportional. Would we be interested to know about it if the equipment were energy proportional?
** Week 4 March
...
- The simulator terminates with the following error as shown at the bottom of [[file:data/eco_val_100.dat]] and [[file:data/eco_val_576.dat]] when the traffic
...
- After a diagnostic run of the previous experiment, Martin explained that the problem seems that when the simulation loops in the same Unix process,
  the consumption of the previous run was carried on to the next run even if *Simulation::Destroy()* was called before the loop ends. 
...
- The following code block runs the modified script to produce the updated raw data files: [[file:data/eco_val_modified_100.dat]],
  [[file:data/eco_val_modified_576.dat]], [[file:data/eco_val_modified_1000.dat]], and [[file:data/eco_val_modified_1500.dat]]
#+BEGIN_SRC bash :results output :exports both
#! /bin/bash
cd ~/ns3workspace/source/ns-3.26
array=(72 548 972 1472)
...
j=$((header + i))
  for ((k=1; k<=30; k++)); do
 ./waf --run "scratch/validate_ecofen_modified --packetSize=${i} --rngRun=${k}" >> ~/internship/data/eco_val_modified_${j}.dat 2>&1
   done
 done
#+END_SRC
- After processing the files produced by the above code block using the python script used in the previous validation experiment, we get the following
  .csv files: [[file:data/eco_val_modified_100.csv]], [[file:data/eco_val_modified_576.csv]], [[file:data/eco_val_modified_1000.csv]], and
  [[file:data/eco_val_modified_1500.csv]]
#+CALL: extract_data(input_file="data/eco_val_modified_100.dat", output_file="data/eco_val_modified_100.csv",size=100) 
#+CALL: extract_data(input_file="data/eco_val_modified_576.dat", output_file="data/eco_val_modified_576.csv",size=576) 
#+CALL: extract_data(input_file="data/eco_val_modified_1000.dat", output_file="data/eco_val_modified_1000.csv",size=1000) 
#+CALL: extract_data(input_file="data/eco_val_modified_1500.dat", output_file="data/eco_val_modified_1500.csv",size=1500) 
...
#+BEGIN_SRC R :results output graphics :file figures/ecof_val_modified_all.png :exports both 
library(ggplot2)
...
#+RESULTS:
[[file:figures/ecof_val_modified_all.png]]

**** Analysis
     - From the plot it seems that the model implemented in ECOFEN is the one for packet size 1000 bytes. For packet size 1000 bytes the simulated
       and expected consumption agrees very well as it is shown in the Unpaired t-test below.
     #+BEGIN_SRC R :results output :var validation=validation :exports both 
       val_1000 = read.csv("data/eco_val_modified_1000.csv", header=T)
       attach(val_1000)
       t.test(Cons_Expected,Cons_Node2,paired=FALSE,conf.level=0.95)
     #+END_SRC
     ...
     data:  Cons_Expected and Cons_Node2
     t = 0.48502, df = 59.292, p-value = 0.6294
     alternative hypothesis: true difference in means is not equal to 0
     95 percent confidence interval:
      -0.04493954  0.07369949
     sample estimates:
     mean of x mean of y 
      10.44507  10.43069 
#+end_example
...
- The p-value tells us that the null hypothesis that the difference in mean is 0 is not rejected. Further more, from the 95% confidence interval we
  can also see that 0 is in the interval which means the difference is by chance or it is as a result of false-positive. 

*** Validating the Complete Model of ECOFEN
- Validating the Complete Model can be done in two steps: validating the model used in the packet receiving side and validating the model used in the
  packet sending side
...
- The objective of this experiment is to compare the implemented models with the ones available in the paper mentioned.
...
- In this experiment, similar to the previous validation experiments, there are three nodes- Node-0, Node-1, and Node-2. Node-0 sends packets at a
  random rate within a range of 1 Mbps and 1800 Mbps. We are interested in the power consumption at Node-0 as a sending end and power consumption at
  Node-2 as a receiving end. 
- Therefore our plan is to compare the power consumption of *Node-0* with the computed consumption from the model shown in [[file:figures/energy_for_transmitting.pdf][Figure 8]] and to compare the
  power consumption of *Node-1* with the computed consumption from the models shown in  [[file:figures/energy_for_recieving.pdf][Figure 7]] 
...
- Running [[file:ns3/validate_ecofen_complete.cc][new NS-3 script]] as shown in the shell code block below, produces [[file:data/eco_val_complete_576.dat]], [[file:data/eco_val_complete_1000.dat]], and
  [[file:data/eco_val_complete_1500.dat]] files.
...
j=$((header + i))
  for ((k=1; k<=50; k++)); do
 ./waf --run "scratch/validate_ecofen_complete --packetSize=${i} --rngRun=${k}" >> ~/internship/data/eco_val_complete_${j}.dat 2>&1
   done
 done
#+END_SRC

- Since the  output data format is different from what we have used in the earlier experiment, the python script for formatting the output is modified
  to accommodate this change as shown below.
#+NAME: extract_complete_data
#+BEGIN_SRC python :results silent dir:~/internship :var input_file="data/eco_val_complete_576.dat" :var output_file="data/eco_val_complete_576.csv" :var size=576
          ...
          with open(output_file,"a") as of:
              if len(data) == 9:
                  if header: # writes the header and the idle consumption
                      ...
                      del data[:]
#+END_SRC
#+CALL: extract_complete_data(input_file="data/eco_val_complete_576.dat", output_file="data/eco_val_complete_576.csv",size=576) 
#+CALL: extract_complete_data(input_file="data/eco_val_complete_1000.dat", output_file="data/eco_val_complete_1000.csv",size=1000) 
#+CALL: extract_complete_data(input_file="data/eco_val_complete_1500.dat", output_file="data/eco_val_complete_1500.csv",size=1500) 
- Running the above python script using the raw data [[file:data/eco_val_complete_576.dat]], [[file:data/eco_val_complete_1000.dat]], and
  [[file:data/eco_val_complete_1500.dat]] gives us the corresponding formatted files: [[file:data/eco_val_complete_576.csv]],
  [[file:data/eco_val_complete_1000.csv]], and [[file:data/eco_val_complete_1500.csv]]. 
**** Validating the receive end 
#+BEGIN_SRC R :results output graphics :file figures/ecof_val_recieve.png :exports both 
library(ggplot2)
...
all_data = rbind(val_576,val_1000,val_1500)
attach(all_data)
ggplot(all_data,aes(x=Mbps))+
   geom_point(aes(y = Cons_Node2, colour = "Simulated Node-2)"))+
   geom_point(aes(y = Cons_ExpecMbps, colour = "Expected"))+
   facet_grid(.~Packet_Size)+
   ylab(label="Power Consumption in Watt")+
   xlab(label="Traffic in Mbps")+
   theme(legend.position="top",
...
#+RESULTS:
[[file:figures/ecof_val_recieve.png]]
  - If we compare the simulated and the expected consumption using unpaired t-test for the middle plot, we get the following  
 #+BEGIN_SRC R :results output :var validation=validation :exports both 
       val_1000 = read.csv("data/eco_val_complete_1000.csv", header=T)
       attach(val_1000)
       t.test(Cons_ExpecMbps,Cons_Node2,paired=FALSE,conf.level=0.95)
#+END_SRC
...
data:  Cons_ExpecMbps and Cons_Node2
t = 17.229, df = 99.318, p-value < 2.2e-16
alternative hypothesis: true difference in means is not equal to 0
95 percent confidence interval:
 0.2996378 0.3776349
sample estimates:
mean of x mean of y 
 10.75146  10.41283 
#+end_example
- the p-value in the above test, which is less than 0.05 for the 95% confidence level, tells us that the null hypothesis that the difference in mean
  is 0 is rejected, which means the difference is true positive or it is not by chance.  
**** Validating the send/transmitting end 
- For the transmitting end there is only one model in the paper mentioned above and the packet size used for that model is 1000. We only used the data
  for packet size 1000 in this case
#+BEGIN_SRC R :results output graphics :file figures/ecof_val_send.png :exports both 
library(ggplot2)
val_1000 = read.csv("data/eco_val_complete_1000.csv", header=T)
attach(val_1000)
ggplot(val_1000,aes(x=Mbps))+
   geom_point(aes(y = Cons_Node0, colour = "Simulated Node-2)"))+
   geom_point(aes(y = Cons_ExpecPps, colour = "Expected"))+
   ylab(label="Power Consumption in Watt")+
...
#+RESULTS:
[[file:figures/ecof_val_send.png]]

  - If we compare the simulated and the expected consumption using unpaired t-test, we get the following
 #+BEGIN_SRC R :results output :var validation=validation :exports both 
       val_1000 = read.csv("data/eco_val_complete_1000.csv", header=T)
       attach(val_1000)
       t.test(Cons_ExpecPps,Cons_Node0,paired=FALSE,conf.level=0.95)
#+END_SRC
...
data:  Cons_ExpecPps and Cons_Node0
t = 7.0977, df = 99.407, p-value = 1.91e-10
alternative hypothesis: true difference in means is not equal to 0
95 percent confidence interval:
 0.1546320 0.2746287
sample estimates:
...
#+end_example
- the p-value shown above, which is less than 0.05 for the 95% confidence level, tells us that the null hypothesis that the difference in mean is 0 is
  rejected, which means the difference is true positive or it is not by chance.
