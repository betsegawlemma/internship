...
- With 95% confidence, we can say that the average difference between the two power consumption estimation lies between 0.05797514  and 0.17852486
- Since we have subtracted the expected measured power consumption value from the simulated value, the positive CI means that, the simulator model
  slightly over estimates the energy consumption
...
**** Analysis
     - From the plot it seems that the model implemented in ECOFEN is the one for packet size 1000 bytes. For packet size 1000 bytes the the simulated
       and expected consumption agrees very well. The 95% CI of the mean difference lies between 0.009692744  and 0.019067202. The mean of the
       difference is 0.0144 as shown below.  
     #+BEGIN_SRC R :results output :var validation=validation :exports both 
       val_1000 = read.csv("data/eco_val_modified_1000.csv", header=T)
       attach(val_1000)
       t.test(Cons_Expected,Cons_Node2,paired=TRUE,conf.level=0.95)
     #+END_SRC
     ...
     t = 6.2655, df = 30, p-value = 6.654e-07
     alternative hypothesis: true difference in means is not equal to 0
     95 percent confidence interval:
      0.009692744 0.019067202
     sample estimates:
     mean of the differences 
                  0.01437997 
...
- Since the traffic unit used in Figure 8's model is packets per second (PPS), the NS-3 script used in this experiment outputs the traffic volume at
  the sending end in terms of PPS for receiving end it outputs in terms of Mbps, that is the unit used in Figure 7.   
- Running [[file:ns3/validate_ecofen_complete.cc][new NS-3 script]] as shown in the shell block below, produces [[file:data/eco_val_complete_576.dat]], [[file:data/eco_val_complete_1000.dat]], and
...
| Received Traffic (in Mbps) | Transmitted Traffic (in PPS) | Node-0 Consumption | Node-1 Consumption | Node-2 Consumption | Total Consumption | Expected Consumption for Receive | Expected Consumption for Transmit | Packet Size |
|----------------------------+------------------------------+--------------------+--------------------+--------------------+-------------------+----------------------------------+-----------------------------------+-------------|
...
**** Analysis
     - In both validation cases there is a constant gap between the simulated and the expected consumption.
