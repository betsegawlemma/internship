...
#+BEGIN_SRC R :results output graphics :file figures/expectedvssimulatedlinear.png :exports both 
library(ggplot2)
library(gridExtra)
...
#+BEGIN_SRC R :results output graphics :file figures/expectedvssimulatedreceiving.png :exports both 
library(ggplot2)
library(gridExtra)
theme_Publication <- function(base_size=14, base_family="helvetica") {
      library(grid)
...
#+BEGIN_SRC R :results output graphics :file figures/expectedvssimulatedsending.png :exports both 
library(ggplot2)
library(gridExtra)
theme_Publication <- function(base_size=14, base_family="helvetica") {
      library(grid)
...
with open(input_file) as fe:
    for line in fe:
       if line.startswith("[H-0:sender:"): # traffic line
           list = line.split()
           if list[TRAFFIC_COL-1] == "traffic":
               traffic = float(list[TRAFFIC_COL]) / 1E6
               data.append(str(traffic))
           elif list[TRAFFIC_COL-1] == "sender":
               done = True 
...
#+BEGIN_SRC R :results output graphics :file figures/transferTime_vs_traffic.png :exports both 
library(ggplot2)
library(gridExtra)
theme_Publication <- function(base_size=14, base_family="helvetica") {
      library(grid)
...
#+BEGIN_EXAMPLE
  Transfer_Time = 0.1525936 * Traffic + 0.1513248
#+END_EXAMPLE
**** Experiment 2: 
...
with open(input_file) as fe:
    for line in fe:
       if line.startswith("[H-0:sender:"): 
           list = line.split()
           if list[SENDER_COL] == "sender":
...
#+BEGIN_SRC R :results output graphics :file figures/transferTime_vs_latency.png :exports both 
library(ggplot2)
library(gridExtra)
theme_Publication <- function(base_size=14, base_family="helvetica") {
      library(grid)
...
**** Experiment 3:
- Running SimGrid with the new flow-level model
#+BEGIN_SRC bash :results output :exports both 
#! /bin/bash
seed=234
RANDOM=$(seed)
...
for ((k=0; k<40; k++)); do
  ...
  cat energy_consumption_d.xml |sed -e "s/\$T/${traffic}E6/" > t_energy_consumption_d.xml
  # run simgrid
  ./sg_energy_consumption energy_consumption_p.xml t_energy_consumption_d.xml >> ~/internship/data/ex3_sg_energy.raw 2>&1
done
#+END_SRC
...
#+BEGIN_SRC R :results output graphics :file figures/energy_sg_vs_ns3.png :exports both 
library(ggplot2)
library(gridExtra)
theme_Publication <- function(base_size=14, base_family="helvetica") {
      library(grid)
...
***** Experiment on larger platform(two nodes, two router, three links) to see if there is similar gap when the topology changes
#+BEGIN_SRC bash :results output :exports both 
#! /bin/bash
seed=234
RANDOM=$(seed)
...
#+BEGIN_SRC R :results output graphics :file figures/ex4_energy_sg_vs_ns3.png :exports both 
library(ggplot2)
library(gridExtra)
theme_Publication <- function(base_size=14, base_family="helvetica") {
      library(grid)
...
**** Experiment 5:
  - the difference growth looks more pronounced with large topology
  - the gap growth looks exponential therefore replacing traffic value in the computation with pow(traffic, 1.08)
#+BEGIN_SRC bash :results output :exports both 
#! /bin/bash
seed=234
RANDOM=$(seed)
...
for ((k=0; k<40; k++)); do
  ...
  cat energy_consumption_d.xml |sed -e "s/\$T/${traffic}E6/" > t_energy_consumption_d.xml
  # run simgrid
  ./sg_energy_consumption energy_consumption_p.xml t_energy_consumption_d.xml >> ~/internship/data/ex5_sg_energy.raw 2>&1
done
#+END_SRC

#+RESULTS:
#+CALL: extract_sg_data(input_file="data/ex5_sg_energy.raw", output_file="data/ex5_sg_energy.csv") 
#+BEGIN_SRC R :results output graphics :file figures/ex5_energy_sg_vs_ns3.png :exports both 
library(ggplot2)
library(gridExtra)
theme_Publication <- function(base_size=14, base_family="helvetica") {
      library(grid)
...
scale_colour_Publication <- function(...){
   ...
   sg_ns3 = merge(ex4_ns3,ex5_sg, by=c("Traffic"))
   attach(sg_ns3)
   ggplot(sg_ns3,aes(x=Traffic))+
   geom_line(aes(y = Total_Energy, colour = "ECOFEN"),size=1.3)+
   geom_line(aes(y = SG_TOTAL_ENERGY, colour = "SimGrid"),size=1.3)+
