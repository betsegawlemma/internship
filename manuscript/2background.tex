...
In this chapter ...
\todo[inline]{Here you need to set the global aim of this chapter and to provide its outline.}

\section{Electricity consumption of ICT equipments}
\label{section:ictequipment} 
\todo[inline]{Equipment is an uncountable noun in English, you should use 'devices' or other synonyms instead.}
...
\todo[inline]{You should also probably give the share between the three categories. Otherwise, the reader is not sure if the 10\% for network is really bad or not.}
...
\todo[inline]{You should add here a paragraph zooming on the energy consumption of network devices as it is the topic of your thesis. Don't forget that your work will be useful for networks inside data centers, but also outside (including networks between data centers belonging to the same distributed cloud infrastructure for instance). You may want to use the following reference: \url{https://eta.lbl.gov/sites/all/files/publications/lbnl-1005775_v2.pdf} and in particular Figure 19 or a close one.}
...
\todo[inline]{You should provide a reference for ALR and one for LPI.}
...
\todo[inline]{I would have put the two first paragraph of this section at the beginning of the previous section as here you are justifying the use of simulators. This should be before the presentation of simulator techniques.}
...
\todo[inline]{The paragraph above should be the first of the section because it justifies why we work with SimGrid.}
...
\todo[inline]{You may want to add a paragraph explaining that these different models try to represent the various behaviors encountered in literature at different levels. For an example for the linear model, you can cite: "Fog Computing May Help to Save Energy in Cloud Computing", F. Jalali et al. IEEE Journal on Selected Areas in Communications, 2016. For the complete model, you can cite~\cite{Sivaraman}.}
...
\todo[inline]{You may also want to add a reference showing that ns3 has a better scalability than ns2. For instance: \url{http://citeseerx.ist.psu.edu/viewdoc/download?doi=10.1.1.468.7549&rep=rep1&type=pdf}}
